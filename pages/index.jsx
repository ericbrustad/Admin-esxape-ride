...
/* =========================================================
   SECTION A — SAFE FETCH HELPERS
   ---------------------------------------------------------
   Prevents "Unexpected token '<'" when a JSON file 404s and
   the server returns HTML. Use these everywhere we fetch JSON.
   ========================================================= */
...
/* =========================================================
   SECTION 0 — TYPE / FORM DEFINITIONS
   ---------------------------------------------------------
   - TYPE_FIELDS drives the generic form renderer (labels+inputs)
   - Some types have custom UIs:
     * multiple_choice -> MultipleChoiceEditor
     * geofence_*      -> MapPicker
     * ar_*            -> geofence toggle + MapPicker + previews
   ========================================================= */
...
const TYPE_FIELDS = {
  ...
  ar_image: [
    ...
    { key: 'overlayText', label: 'Overlay Text (optional)', type: 'text' },
    // geofence handled below via toggle + MapPicker
  ],
  ar_video: [
    { key: 'markerUrl', label: 'AR Marker Image URL (png/jpg)', type: 'text' },
...
export default function Admin() {
  ...
  // SECTION 1.B — Slug-aware loader for missions/config
  useEffect(() => {
    (async () => {
      try {
        setStatus('');

        // Try game-specific missions first, then fall back to root missions.json
        const missionUrls = activeSlug
          ? [
              `/games/${encodeURIComponent(activeSlug)}/missions.json`,
              `/missions.json`,
            ]
          : [`/missions.json`];
  ...
  function saveToList() {
    ...
    // presence checks for non-number fields from schema
      ...
      <header style={S.header}>
        <div style={S.wrap}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <a
              href={activeSlug ? `/games/${activeSlug}/config.json` : `/config.json`}
              target="_blank"
              rel="noreferrer"
              style={{ ...S.button }}
            >
                  ...
                  <Field key={f.key} label={f.label}>
                    {f.type === 'text' && (
                      <>
                        ...
                        />
                        {/* Inline previews for known media-like keys */}
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const rr = await fetch('/api/games', { cache: 'no-store' });
                    const jj = await rr.json();
                    if (jj.ok) setGames(jj.games || []);
                  } catch {}
                  setActiveSlug(j.slug || j.game?.slug || '');
                  setNewTitle('');
  ...
  return (
    <div style={{ border: '1px solid #2a323b', borderRadius: 10, padding: 12 }}>
      ...
      {[0, 1, 2, 3, 4].map((i) => (
        <div
          key={i}
          style={{ display: 'grid', gridTemplateColumns: '24px 1fr', alignItems: 'center', gap: 8, marginBottom: 8 }}
        >
          <input
            type="radio"
            name="mcq-correct"
...
function MapPicker({ lat, lng, radius, onChange }) {
  ...
  function gotoResult(r) {
    ...
    onChange(Number(lat2.toFixed(6)), Number(lon2.toFixed(6)), Number(r || 25));
    setResults([]);
  }
