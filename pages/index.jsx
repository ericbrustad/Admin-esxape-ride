...
function isAudioUrl(u) {
  if (!u) return false;
  return /\.(mp3|wav|m4a|aac|ogg)(\?|#|$)/i.test(u);
}
...
const TYPE_FIELDS = {
  ...
  statement: [
    ...
    { key: 'mediaUrl', label: 'Image, Video, or Audio URL (optional)', type: 'text' },
  ],
  stored_statement: [
  ...
  geofence_image: [
    ...
    { key: 'cooldownSeconds', label: 'Cooldown (sec)', type: 'number', min: 5, max: 3600 },
...
function defaultAppearance() {
  return {
    textAlign: 'center', // 'top' or 'center'
...
export default function Admin() {
  ...
  // media
  const [globalMedia, setGlobalMedia] = useState([]); // [{key,name,url,type}]
  const [gameMedia, setGameMedia] = useState([]);     // game-specific
  const [chooserOpen, setChooserOpen] = useState(false);
  const [chooserFilter, setChooserFilter] = useState('all'); // all|image|video|audio|gif
  const [onPickMedia, setOnPickMedia] = useState(null);

  // compute game origin (for embedded test)
  const [effectiveGameBase, setEffectiveGameBase] = useState('');
  useEffect(() => {
    const base = (typeof window !== 'undefined'
      ? (window.__GAME_ORIGIN__ || process.env.NEXT_PUBLIC_GAME_ORIGIN)
      : process.env.NEXT_PUBLIC_GAME_ORIGIN) || '';
    setEffectiveGameBase(base);
  }, []);
  ...
  // load suite/config + media indexes per slug
  useEffect(() => {
    (async () => {
      try {
        setStatus('Loading…');
        ...
        setConfig({
          ...
          rewards: Array.isArray(c.rewards) ? c.rewards : [],     // NEW
          timer: { ...(defaultConfig().timer), ...(c.timer || {}) },
        });

        // load media indexes (best-effort)
        const gm = activeSlug
          ? await fetchJsonSafe(`/games/${encodeURIComponent(activeSlug)}/media/index.json`, [])
          : await fetchJsonSafe(`/media/index.json`, []);
        const glob = await fetchJsonSafe(`/media-pool/index.json`, []);
        setGameMedia(Array.isArray(gm) ? gm : []);
        setGlobalMedia(Array.isArray(glob) ? glob : []);
  ...
  function defaultConfig() {
    return {
      ...
      textRules: [],
      powerups: [],
      rewards: [],        // [{name, ability, thumbUrl}]
      mapOverlay: '',     // optional tile overlay id/url
    };
  }
    ...
    switch (t) {
      case 'multiple_choice':
        return {
          ...
          mediaUrl: '',
          requireAck: false,
          onCorrect: { enabled: false, mediaUrl: '', text: '', appearance: defaultAppearance() },
          onWrong:   { enabled: false, mediaUrl: '', text: '', appearance: defaultAppearance() },
          ...baseGeo
        };
      case 'short_answer':
        return { question: '', answer: '', acceptable: '', mediaUrl: '', requireAck: false, ...baseGeo };
      case 'statement':
        return { text: '', mediaUrl: '', requireAck: true, ...baseGeo };
      case 'stored_statement':
        return { template: '', requireAck: true, ...baseGeo };
      case 'video':
        return { videoUrl: '', overlayText: '', requireAck: true, ...baseGeo };
      case 'geofence_image':
      ...
      case 'ar_image':
        return { markerUrl: '', assetUrl: '', overlayText: '', requireAck: false, ...baseGeo };
      case 'ar_video':
        return { markerUrl: '', assetUrl: '', overlayText: '', requireAck: false, ...baseGeo };
      default:
        return { ...baseGeo };
  ...
  async function saveAll() {
    ...
    if (!ok) {
      ...
      setStatus('✅ Saved (files committed). Rebuilding…');
    }
  }
  ...
  function editExisting(m) {
    ...
    const safe = {
      ...
      rewards: { points: Number(m?.rewards?.points ?? 0) },
    };
  ...
  function duplicateMission(idx) {
    ...
    next.id = suggestId();
    next.title = (src.title || 'Mission') + ' (copy)';
    list.splice(idx + 1, 0, next);
    setSuite({ ...suite, missions: list, version: bumpVersion(suite.version || '0.0.0') });
  }
  ...
  // media chooser
  function openMediaChooser(onPick, filter = 'all') {
    setOnPickMedia(() => onPick);
    setChooserFilter(filter);
    setChooserOpen(true);
  }
  function filteredMedia(list) {
    if (chooserFilter === 'all') return list;
    return list.filter((m) => {
      const u = (m.url || '').toLowerCase();
      if (chooserFilter === 'image') return isImageUrl(u);
      if (chooserFilter === 'video') return isVideoUrl(u);
      if (chooserFilter === 'audio') return isAudioUrl(u);
      if (chooserFilter === 'gif')   return /\.gif(\?|#|$)/i.test(u);
      return true;
    });
  }

  if (!suite || !config) {
    return <main style={S.wrap}><div style={S.card}>Loading…</div></main>;
  }
  ...
  return (
    <div style={S.body}>
      <header style={S.header}>
        <div style={S.wrap}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            {['settings', 'missions', 'text', 'rewards', 'devices', 'media', 'map', 'test'].map((t) => (
              ...
              {(suite.missions || []).map((m, idx) => (
                <div key={m.id} data-m-title={(m.title || '') + ' ' + m.id + ' ' + m.type} style={S.missionItem}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 8, alignItems: 'center' }}>
                    {/* Delete left */}
                    <button title="Delete" style={{ ...S.button, padding: '6px 10px' }} onClick={() => removeMission(m.id)}>Delete</button>

                    <div onClick={() => editExisting(m)} style={{ cursor: 'pointer' }}>
                      <div style={{ fontWeight: 600 }}>
                        <span style={{ opacity:.6, marginRight:6 }}>#{String(idx+1).padStart(2,'0')}</span>
                        {m.title || m.id}
                      </div>
                    ...
                    <div style={{ display:'flex', gap:6 }}>
                      <button title="Move up"   disabled={idx===0} style={{ ...S.button, padding:'6px 10px' }} onClick={()=>moveMission(idx, -1)}>↑</button>
                      <button title="Move down" disabled={idx===(suite.missions.length-1)} style={{ ...S.button, padding:'6px 10px' }} onClick={()=>moveMission(idx, +1)}>↓</button>
              ...
              <div style={S.card}>
                ...
                <Field label="Type">
                  <select
                    ...
                    onChange={(e) => {
                      const t = e.target.value || 'multiple_choice';
                ...
                {editing.type === 'multiple_choice' && (
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
                      <CelebrationEditor
                        label="On Correct"
                        value={editing.content?.onCorrect || { enabled:false, mediaUrl:'', text:'', appearance: defaultAppearance() }}
                        openChooser={(filter, cb) => openMediaChooser(cb, filter)}
                        onChange={(v) => { setEditing({ ...editing, content: { ...editing.content, onCorrect: v } }); setDirty(true); }}
                      />
                      <CelebrationEditor
                        label="On Wrong"
                        value={editing.content?.onWrong || { enabled:false, mediaUrl:'', text:'', appearance: defaultAppearance() }}
                        openChooser={(filter, cb) => openMediaChooser(cb, filter)}
                        onChange={(v) => { setEditing({ ...editing, content: { ...editing.content, onWrong: v } }); setDirty(true); }}
                      />
                    </div>
                  </div>
                )}

                {/* Stored Statement helper */}
                  ...
                  <div style={{ marginBottom: 12, border:'1px solid #22303c', borderRadius:10, padding:12 }}>
                    ...
                    <Field label="Template (click IDs below to insert a tag like #m03# where your cursor is)">
                      <textarea
                        ...
                        value={(editing.content && editing.content.template) || ''}
                        onChange={(e)=>{ setEditing({ ...editing, content:{ ...(editing.content||{}), template:e.target.value } }); setDirty(true); }}
                      />
                    </Field>
                    <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                      {(suite.missions||[]).map((mm) => (
                        <button key={mm.id} style={S.button} type="button"
                          onClick={()=>{
                            const tag = `#${mm.id}#`;
                            const cur = (editing.content?.template || '');
                            setEditing({ ...editing, content:{ ...(editing.content||{}), template: cur + ' ' + tag } });
                            setDirty(true);
                          }}
                        >{mm.id}</button>
                      ))}
                ...
                {/* Universal geofence toggle for content types */}
                {(editing.type === 'multiple_choice' || editing.type === 'short_answer' || editing.type === 'statement' || editing.type === 'stored_statement' || editing.type === 'video') && (
                  <div style={{ marginBottom: 12 }}>
                    <label style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                      <input
                        type="checkbox"
                          ...
                          <div style={{ display:'flex', gap:8, alignItems:'center', marginTop:8 }}>
                            ...
                            <button type="button" style={S.button}
                              onClick={()=>openMediaChooser(
                                (m)=>{ setEditing({ ...editing, content:{ ...editing.content, [f.key]: m.url } }); setDirty(true); },
                                f.key.includes('video') ? 'video' : (f.key.includes('image') || f.key==='mediaUrl' ? 'image' : 'all')
                              )}
                ...
                {/* Require acknowledge (X) */}
                {['statement', 'stored_statement', 'video'].includes(editing.type) && (
                  <Field label="Require player to acknowledge (X button)">
                    <label style={{ display:'flex', gap:8, alignItems:'center' }}>
                      <input
                        type="checkbox"
                        checked={!!editing.content?.requireAck}
                        onChange={(e)=>{ setEditing({ ...editing, content:{ ...(editing.content||{}), requireAck: e.target.checked } }); setDirty(true); }}
                      />
                      Show ✕ and require tap to continue
                    </label>
                  </Field>
                )}
                ...
                <details style={{ marginTop: 12 }}>
                  <summary style={{ cursor:'pointer', userSelect:'none' }}><b>Use custom appearance for this mission</b></summary>
                  <div style={{ marginTop: 8 }}>
                    ...
                    {editing.appearanceOverrideEnabled && (
                      <AppearanceEditor
                        ...
                        onChange={(v)=>{ setEditing({ ...editing, appearance: v }); setDirty(true); }}
                      />
                    )}
                  </div>
                </details>
        ...
        <main style={S.wrap}>
          <div style={S.card}>
            ...
            <Field label="Duration (minutes — 0 = infinite; count UP)">
              <input
                type="number" min={0} max={24 * 60} style={S.input}
            ...
            <Field label="Alert before end (minutes — chime + warning)">
              <input
                type="number" min={1} max={120} style={S.input}
            ...
            <hr style={S.hr} />
            <h4>Appearance (Global)</h4>
            <AppearanceEditor
              value={config.appearance || defaultAppearance()}
              onChange={(v)=> setConfig({ ...config, appearance: v })}
            />
          </div>
        ...
        <main style={S.wrap}>
          <div style={S.card}>
            ...
            <div style={{ display:'grid', gap:12 }}>
              {(config.rewards || []).map((rw, idx) => (
                <div key={idx} style={{ border:'1px solid #22303c', borderRadius:10, padding:12 }}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                    <Field label="Name">
                      <input style={S.input} value={rw.name||''}
                        onChange={(e)=> {
                          const list = [...(config.rewards||[])];
                          list[idx] = { ...rw, name:e.target.value };
                          setConfig({ ...config, rewards:list });
                        }} />
                    </Field>
                    <Field label="Special ability">
                      <input style={S.input} value={rw.ability||''}
                        onChange={(e)=> {
                          const list = [...(config.rewards||[])];
                          list[idx] = { ...rw, ability:e.target.value };
                          setConfig({ ...config, rewards:list });
                        }} />
                    </Field>
                  </div>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:12, alignItems:'center', marginTop:8 }}>
                    <Field label="Thumbnail (URL)">
                      <input style={S.input} value={rw.thumbUrl||''}
                        onChange={(e)=> {
                          const list = [...(config.rewards||[])];
                          list[idx] = { ...rw, thumbUrl:e.target.value };
                          setConfig({ ...config, rewards:list });
                        }} />
                    </Field>
                    <button type="button" style={S.button}
                      onClick={()=> openMediaChooser(
                        (m)=>{
                          const list = [...(config.rewards||[])];
                          list[idx] = { ...rw, thumbUrl: m.url };
                          setConfig({ ...config, rewards:list });
                        }, 'image'
                      )}
                    >Pick from Media Pool</button>
                  </div>

                  <div style={{ marginTop:8 }}>
                    <MediaPreview url={rw.thumbUrl} kind="reward thumb" />
                  </div>

                  <div style={{ display:'flex', gap:8, marginTop:10 }}>
                    <button style={S.button} onClick={()=>{
                      const list = [...(config.rewards||[])]; list.splice(idx,1); setConfig({ ...config, rewards:list });
                    }}>Remove</button>
                    <button style={S.button} onClick={()=>{
                      const copy = { ...(rw||{}) };
                      const list = [...(config.rewards||[])];
              ...
              <button style={S.button} onClick={()=>{
                const list = [...(config.rewards||[])];
                list.push({ name:'', ability:'', thumbUrl:'' });
                setConfig({ ...config, rewards:list });
              }}>+ Add Reward</button>
            </div>
          </div>
      ...
      {tab === 'devices' && (
        <main style={S.wrap}>
          <div style={S.card}>
            <h3 style={{ marginTop: 0 }}>Devices (Power‑Ups)</h3>
            ...
            {(config.powerups || []).length === 0 && <div style={{ color: '#9fb0bf' }}>No devices yet.</div>}
            <ul style={{ paddingLeft: 18 }}>
          ...
          <div style={S.card}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
              ...
              <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                <label>Filter:&nbsp;
                  <select style={S.input} value={chooserFilter} onChange={(e)=>setChooserFilter(e.target.value)}>
                ...
                <small style={{ color:'#9fb0bf' }}>Global pool + game media shown.</small>
              </div>
            </div>

            <div style={{ marginBottom:12 }}>
              <h4>Game Media</h4>
              <ThumbGrid items={filteredMedia(gameMedia)} />
              {filteredMedia(gameMedia).length === 0 && <div style={{ color:'#9fb0bf' }}>No media in this game.</div>}
            </div>

            <div>
              <h4>Global Media Pool</h4>
              <ThumbGrid items={filteredMedia(globalMedia)} />
              {filteredMedia(globalMedia).length === 0 && <div style={{ color:'#9fb0bf' }}>No items in the global pool.</div>}
            </div>

            <hr style={S.hr} />
            <details>
              <summary style={{ cursor:'pointer' }}><b>Add by URL</b> (optional; requires API on your side)</summary>
              <UrlAddForm slug={activeSlug} onStatus={setStatus} />
            </details>
          </div>
        </main>
      )}
      ...
      {tab === 'map' && (
        <main style={S.wrap}>
          <div style={S.card}>
            ...
            <MapOverview missions={(suite?.missions)||[]} devices={(config?.powerups)||[]} showRings={showRings} />
            <div style={{ color: '#9fb0bf', marginTop: 8 }}>Shows all geofenced missions and devices for the selected game.</div>
          </div>
        </main>
      )}
      ...
      {tab === 'test' && (
        <main style={S.wrap}>
          <div style={S.card}>
            ...
            {!effectiveGameBase && <div style={{ color:'#9fb0bf', marginBottom:8 }}>Set NEXT_PUBLIC_GAME_ORIGIN (or config.gameOrigin) to enable embedded preview.</div>}
            {effectiveGameBase && (
              <iframe
                src={`${effectiveGameBase}/?slug=${activeSlug || ''}&channel=${testChannel}&preview=1`}
      ...
      {/* Media chooser overlay */}
      {chooserOpen && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.6)', display:'grid', placeItems:'center', zIndex:2000 }}>
          <div style={{ ...S.card, width:'min(980px, 94vw)', maxHeight:'86vh', overflow:'auto' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
              <h3 style={{ margin:0 }}>Pick from Media</h3>
              <button style={S.button} onClick={()=>setChooserOpen(false)}>Close</button>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
              <div>
                <h4>Game Media</h4>
                <ThumbGrid items={filteredMedia(gameMedia)} onPick={(m)=>{ onPickMedia && onPickMedia(m); setChooserOpen(false); }} pickLabel="Use" />
              </div>
              <div>
                <h4>Global Pool</h4>
                <ThumbGrid items={filteredMedia(globalMedia)} onPick={(m)=>{ onPickMedia && onPickMedia(m); setChooserOpen(false); }} pickLabel="Use" />
              </div>
            </div>
          </div>
        </div>
...
function AppearanceEditor({ value, onChange }) {
  const v = value || defaultAppearance();
  return (
    <div style={{ border:'1px solid #22303c', borderRadius:10, padding:12 }}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
        <Field label="Text alignment">
          <select style={S.input} value={v.textAlign} onChange={(e)=>onChange({ ...v, textAlign: e.target.value })}>
            <option value="top">Top</option>
            <option value="center">Center</option>
          </select>
        </Field>
        <Field label="Font family">
          <input style={S.input} value={v.fontFamily} onChange={(e)=>onChange({ ...v, fontFamily: e.target.value })}/>
        </Field>
        <Field label="Font size (px)">
          <input type="number" min={10} max={80} style={S.input} value={v.fontSizePx}
                 onChange={(e)=>onChange({ ...v, fontSizePx: Number(e.target.value||22) })}/>
        </Field>
        <Field label="Font color">
          <input style={S.input} value={v.fontColor} onChange={(e)=>onChange({ ...v, fontColor: e.target.value })}/>
        </Field>
        <Field label="Font background color">
          <input style={S.input} value={v.fontBgColor} onChange={(e)=>onChange({ ...v, fontBgColor: e.target.value })}/>
        </Field>
        <Field label="Font background opacity (0–1)">
          <input type="number" step="0.05" min={0} max={1} style={S.input} value={v.fontBgAlpha}
                 onChange={(e)=>onChange({ ...v, fontBgAlpha: Math.max(0, Math.min(1, Number(e.target.value||0))) })}/>
        </Field>
        <Field label="Screen background color">
          <input style={S.input} value={v.screenBgColor} onChange={(e)=>onChange({ ...v, screenBgColor: e.target.value })}/>
        </Field>
        <Field label="Screen background opacity (0–1)">
          <input type="number" step="0.05" min={0} max={1} style={S.input} value={v.screenBgAlpha}
                 onChange={(e)=>onChange({ ...v, screenBgAlpha: Math.max(0, Math.min(1, Number(e.target.value||0))) })}/>
        </Field>
        <Field label="Screen background image URL (optional)">
          <input style={S.input} value={v.screenBgImageUrl}
                 onChange={(e)=>onChange({ ...v, screenBgImageUrl: e.target.value })}/>
        </Field>
      </div>
    </div>
  );
...
function CelebrationEditor({ label, value, onChange, openChooser }) {
  const v = value || { enabled:false, mediaUrl:'', text:'', appearance: defaultAppearance() };
  return (
    <div style={{ border:'1px solid #22303c', borderRadius:10, padding:12 }}>
      ...
      {v.enabled && (
        <>
          <Field label="Media URL (image / video / gif / audio)">
            <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:8 }}>
              <input style={S.input} value={v.mediaUrl||''} onChange={(e)=>onChange({ ...v, mediaUrl:e.target.value })}/>
              <button type="button" style={S.button} onClick={()=>openChooser('all', (m)=> onChange({ ...v, mediaUrl: m.url }))}>Pick</button>
            </div>
            <div style={{ marginTop:8 }}><MediaPreview url={v.mediaUrl} kind="celebration" /></div>
          </Field>
          <Field label="Message text (optional)">
            <input style={S.input} value={v.text||''} onChange={(e)=>onChange({ ...v, text:e.target.value })}/>
          </Field>
          <Field label="Appearance (for the message)">
            <AppearanceEditor value={v.appearance || defaultAppearance()} onChange={(a)=>onChange({ ...v, appearance:a })}/>
          </Field>
        </>
      )}
    </div>
...
function MediaPreview({ url, kind }) {
  ...
  const isVid = isVideoUrl(lower);
  const isImg = isImageUrl(lower);
  const isAud = isAudioUrl(lower);
  return (
    <div style={{ marginTop: 8 }}>
...
function MapPicker({ lat, lng, radius, onChange }) {
  ...
  useEffect(() => {
    ...
    const s = document.createElement('script'); s.src = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.js'; s.async = true; s.onload = () => setReady(true);
...
/* =====================================================================
   Thumb grid (Media)
   ===================================================================== */
function ThumbGrid({ items = [], onPick = null, pickLabel = 'Pick' }) {
  return (
    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(140px, 1fr))', gap:10 }}>
      {items.map((m) => {
        const u = toDirectMediaURL(m.url||'');
        const vid = isVideoUrl(u), img = isImageUrl(u), aud = isAudioUrl(u);
        return (
          <div key={m.key||m.url} style={{ border:'1px solid #22303c', borderRadius:10, overflow:'hidden', background:'#0b0c10' }}>
            <div style={{ height:100, display:'grid', placeItems:'center', overflow:'hidden' }}>
              {img ? <img src={u} alt={m.name||''} style={{ width:'100%', height:'100%', objectFit:'cover' }}/>
                : vid ? <div style={{ fontSize:32, opacity:.7 }}>🎬</div>
                : aud ? <div style={{ fontSize:32, opacity:.7 }}>🎵</div>
                : <div style={{ fontSize:28, opacity:.7 }}>🗂</div>}
            </div>
            <div style={{ padding:'6px 8px', fontSize:12 }}>
              <div style={{ whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{m.name || (m.url||'').split('/').pop()}</div>
              <div style={{ display:'flex', gap:6, marginTop:6 }}>
                {onPick && <button style={S.button} onClick={()=>onPick(m)}>{pickLabel}</button>}
                <button style={S.button} onClick={()=>navigator.clipboard?.writeText(u)}>Copy URL</button>
                <a href={u} target="_blank" rel="noreferrer" style={{ ...S.button, textDecoration:'none' }}>Open</a>
              </div>
            </div>
          </div>
        );
...
function UrlAddForm({ slug, onStatus }) {
  const [url, setUrl] = useState('');
  const [scope, setScope] = useState('game'); // 'game' | 'global'
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr auto auto', gap:8, marginTop:8 }}>
      <input placeholder="https://..." style={S.input} value={url} onChange={(e)=>setUrl(e.target.value)} />
      <select style={S.input} value={scope} onChange={(e)=>setScope(e.target.value)}>
        <option value="game">Add to this game</option>
        <option value="global">Add to global pool</option>
      </select>
      <button style={S.button} onClick={async ()=>{
        if (!url.trim()) return;
        try {
          onStatus?.('Adding media…');
          const res = await fetch('/api/media/add', {
            method:'POST',
            headers:{ 'Content-Type':'application/json' },
            body: JSON.stringify({ url, scope, slug })
          });
          const txt = await res.text();
          if (!res.ok) throw new Error(txt || 'Add failed');
          onStatus?.('✅ Added. Save All to commit.');
        } catch(e) {
...
/* =====================================================================
   MapOverview — all missions + devices
   ===================================================================== */
function MapOverview({ missions = [], devices = [], showRings = true }) {
  ...
  React.useEffect(() => {
    ...
    const deviceIcon  = L.divIcon({ className: 'power-icon',   html: '<div style="width:18px;height:18px;border-radius:4px;background:#f59e0b;border:2px solid white;box-shadow:0 0 0 2px #1f2937"></div>' });
    ...
    (devices||[]).forEach((p) => {
      ...
      const mk = L.marker(pos, { icon: deviceIcon }).addTo(layer);
      const title = p.title || p.type || 'Device';
  ...
  }, [leafletReady, missions, devices, showRings]);
      ...
      {((missions||[]).filter(m => (m.content?.geofenceEnabled || Number(m.content?.radiusMeters) > 0)).length === 0) &&
       ((devices||[]).length === 0) && (
        <div style={{ color: '#9fb0bf', marginTop: 8 }}>
          No geofenced missions or devices found. Enable a mission’s geofence (lat/lng &amp; radius) or add devices with lat/lng.
        </div>
      )}
    </div>
  );
