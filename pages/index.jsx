...
export default function Admin() {
  ...
  const _envGameOrigin = (typeof window !== 'undefined'
  ...
  const [gameOriginOverride, setGameOriginOverride] = useState(
    (typeof window !== 'undefined' && window.localStorage) ? (window.localStorage.getItem('admin_game_origin') || '') : ''
  );
  useEffect(() => { try { if (typeof window !== 'undefined') window.localStorage.setItem('admin_game_origin', gameOriginOverride || ''); } catch {} }, [gameOriginOverride]);

  const gameBase = (gameOriginOverride || _envGameOrigin);
                ...
                <div key={m.id} data-m-title={(m.title || '') + ' ' + m.id + ' ' + m.type} style={S.missionItem}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                    ...
                    <div style={{ display:'flex', gap:6 }}>
                      <button style={{ ...S.button, padding: '6px 10px' }} title="Move up"
                        onClick={(e)=>{ e.stopPropagation(); setSuite(s => ({ ...s, missions: moveMissionById((s.missions||[]), m.id, 'up') })); setDirty(true); }}>↑</button>
                      <button style={{ ...S.button, padding: '6px 10px' }} title="Move down"
                        onClick={(e)=>{ e.stopPropagation(); setSuite(s => ({ ...s, missions: moveMissionById((s.missions||[]), m.id, 'down') })); setDirty(true); }}>↓</button>
                      <button style={{ ...S.button, padding: '6px 10px' }} onClick={(e)=>{ e.stopPropagation(); removeMission(m.id); }}>
                        Delete
                      </button>
                    </div>
                  </div>
          ...
          <section style={S.editor}>
            {!editing ? (
              ...
              <div style={S.card}>
                ...
                {/* Appearance */}
                <fieldset style={{ marginTop: 12, border: '1px solid #1f262d', borderRadius: 10, padding: 10 }}>
                  <legend style={{ padding:'0 6px', color:'#9fb0bf' }}>Appearance</legend>
                  <label style={{ display:'flex', gap:8, alignItems:'center', marginBottom:8 }}>
                    <input type="checkbox"
                      checked={!!editing.content?.styleEnabled}
                      onChange={(e)=>{ const on=e.target.checked; setEditing({ ...editing, content:{ ...editing.content, styleEnabled:on, style: (editing.content?.style||{}) } }); setDirty(true); }}
                    />
                  ...
                  {editing.content?.styleEnabled && (
                    <>
                      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                        <div>
                          <div style={{ fontSize:12, color:'#9fb0bf', marginBottom:4 }}>Font</div>
                          <select
                            style={S.input}
                            value={editing.content?.style?.fontFamily || ''}
                            onChange={(e)=>{ setEditing({ ...editing, content:{ ...editing.content, style:{ ...(editing.content?.style||{}), fontFamily:e.target.value } } }); setDirty(true); }}
                          >
                            <option value="">(inherit)</option>
                            <option value='system-ui, -apple-system, Segoe UI, Roboto, Arial, Helvetica'>System Sans</option>
                          ...
                          <div style={{ marginTop:6, padding:8, border:'1px solid #2a323b', borderRadius:8, fontFamily: editing.content?.style?.fontFamily || 'inherit' }}>
                            Sample: AaBbCc123
                          </div>
                        </div>

                        <div>
                          <div style={{ fontSize:12, color:'#9fb0bf', marginBottom:4 }}>Font size</div>
                          <input
                            type="number" min="10" max="64" step="1"
                            style={S.input}
                            value={editing.content?.style?.fontSize ?? ''}
                            onChange={(e)=>{ const v = Number(e.target.value)||undefined; setEditing({ ...editing, content:{ ...editing.content, style:{ ...(editing.content?.style||{}), fontSize:v } } }); setDirty(true); }}
                          />
                        </div>

                        <div>
                          <div style={{ fontSize:12, color:'#9fb0bf', marginBottom:4 }}>Text color</div>
                          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                            {['#e9eef2','#ffffff','#cbd5e1','#94a3b8','#64748b','#1f2937','#0b0c10','#000000','#f87171','#fb923c','#fbbf24','#34d399','#60a5fa','#a78bfa','#f472b6'].map(c => (
                              <button key={c} onClick={()=>{ setEditing({ ...editing, content:{ ...editing.content, style:{ ...(editing.content?.style||{}), textColor:c } } }); setDirty(true); }}
                                style={{ width:22, height:22, borderRadius:6, border:'1px solid #2a323b', background:c, cursor:'pointer' }} title={c} />
                            ))}
                          </div>
                          <input style={{ ...S.input, marginTop:6 }} placeholder="#RRGGBB"
                            value={editing.content?.style?.textColor || ''}
                            onChange={(e)=>{ setEditing({ ...editing, content:{ ...editing.content, style:{ ...(editing.content?.style||{}), textColor:e.target.value } } }); setDirty(true); }}
                          />
                        </div>

                        <div>
                          <div style={{ fontSize:12, color:'#9fb0bf', marginBottom:4 }}>Background color</div>
                          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                            {['#0b0c10','#000000','#1f2937','#64748b','#94a3b8','#cbd5e1','#ffffff','#e9eef2','#f87171','#fb923c','#fbbf24','#34d399','#60a5fa','#a78bfa','#f472b6'].map(c => (
                              <button key={c} onClick={()=>{ setEditing({ ...editing, content:{ ...editing.content, style:{ ...(editing.content?.style||{}), backgroundColor:c } } }); setDirty(true); }}
                                style={{ width:22, height:22, borderRadius:6, border:'1px solid #2a323b', background:c, cursor:'pointer' }} title={c} />
                            ))}
                          </div>
                          <input style={{ ...S.input, marginTop:6 }} placeholder="#RRGGBB"
                            value={editing.content?.style?.backgroundColor || ''}
                            onChange={(e)=>{ setEditing({ ...editing, content:{ ...editing.content, style:{ ...(editing.content?.style||{}), backgroundColor:e.target.value } } }); setDirty(true); }}
                          />
                        </div>

                        <div style={{ gridColumn:'1 / span 2' }}>
                          <div style={{ fontSize:12, color:'#9fb0bf', marginBottom:4 }}>Background image URL</div>
                          <input style={S.input} placeholder="https://…"
                            value={editing.content?.style?.backgroundImageUrl || ''}
                            onChange={(e)=>{ setEditing({ ...editing, content:{ ...editing.content, style:{ ...(editing.content?.style||{}), backgroundImageUrl:e.target.value } } }); setDirty(true); }}
                          />
                        </div>

                        <div>
                          <div style={{ fontSize:12, color:'#9fb0bf', marginBottom:4 }}>Background size</div>
                          <input style={S.input} placeholder="cover | contain | 100% 100%"
                            value={editing.content?.style?.backgroundSize || ''}
                            onChange={(e)=>{ setEditing({ ...editing, content:{ ...editing.content, style:{ ...(editing.content?.style||{}), backgroundSize:e.target.value } } }); setDirty(true); }}
                          />
                        </div>
                      </div>
                    </>
          ...
          <div style={S.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                ...
                <label>Game origin:&nbsp;
                  <input value={gameOriginOverride} onChange={(e)=>setGameOriginOverride(e.target.value.trim())}
                         placeholder="https://esxaperide.com" style={S.input} />
                </label>
            ...
            {(typeof window !== 'undefined') && gameBase && window.location.protocol === 'https:' && /^http:/.test(gameBase) && (
              <div style={{ color:'#ffd18a', background:'#251a00', border:'1px solid #5b3b00', borderRadius:12, padding:8, marginBottom:8 }}>
                HTTPS admin cannot embed HTTP game origin. Change the origin to <code>https://…</code> or click <b>Open full window</b>.
              </div>
            )}
...
function MapOverview({ missions=[], powerups=[], showRings=true }) {
  ...
  React.useEffect(() => {
    ...
    (missions||[]).forEach((m) => {
      const pos = getLL(m);
      const c = (m && m.content) || {};
      const hasCoords = !!pos; const isFence = !!(c.geofenceEnabled || Number(c.radiusMeters) > 0 || hasCoords);
