...
async function createResponseMission(type, responsePayload) {
  try {
    const game = suite || config || (games && games[0]) || null;
    if (!game) return null;
    ...
    if (typeof setSuite === 'function' && suite) {
      setSuite(prev => ({ ...prev, missions: missions }));
    } else if (typeof setConfig === 'function' && config) {
      setConfig(prev => ({ ...prev, missions: missions }));
    } else if (typeof setGames === 'function' && games) {
      setGames(prev => {
        const next = [...(prev || [])];
        if (next[0]) next[0].missions = missions;
        return next;
      });
    }
    return newMission;
