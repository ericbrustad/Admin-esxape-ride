...
/* =========================================================
   0) SMALL UTILS (top-level helpers)
   ========================================================= */

// Fetch JSON safely with a fallback (avoids HTML error pages blowing up .json())
async function fetchJsonSafe(url, fallback) {
  try {import { useEffect, useMemo, useRef, useState } from 'react';

/* =========================================================
   Helpers (top-level, safe to reuse)
   ========================================================= */
function normalizeMediaUrl(u) {
  ...
  try {
    ...
    // Dropbox shared → direct
    if (url.hostname.endsWith('dropbox.com')) {
      // https://www.dropbox.com/s/xyz/file.png?dl=0 → dl.dropboxusercontent.com/s/xyz/file.png
    ...
    // Google Drive file viewer → direct
...
function setQS(next) {
  ...
  const u = `${window.location.pathname}?${cur.toString()}${window.location.hash || ''}`;
  window.history.replaceState({}, '', u);
}

/* =========================================================
   0) TYPE / FORM DEFINITIONS
   ---------------------------------------------------------
   - TYPE_FIELDS drives the generic form renderer
   - Custom UIs:
     * multiple_choice -> MultipleChoiceEditor
     * geofence_*      -> MapPicker
     * ar_*            -> geofence toggle + MapPicker
     * MC/SA/Statement -> optional geofence toggle + MapPicker
   ========================================================= */
...
const TYPE_FIELDS = {
  ...
  // NEW: power-up stash mission types
  powerup_smoke: [
    { key:'lat', label:'Latitude', type:'number' },
    { key:'lng', label:'Longitude', type:'number' },
    { key:'radiusMeters', label:'Pickup Radius (m)', type:'number', min:3, max:100 },
    { key:'replenishSec', label:'Replenish After (sec)', type:'number', min:0, max:3600 },
    { key:'limitPerPlayer', label:'Limit Per Player', type:'number', min:0, max:99 },
    { key:'iconUrl', label:'Map Icon URL (optional)', type:'text' },
  ],
  powerup_clone: [
    { key:'lat', label:'Latitude', type:'number' },
    { key:'lng', label:'Longitude', type:'number' },
...
const TYPE_OPTIONS = [
  ...
  { value:'powerup_smoke', label:'Power-Up: Smoke' },
  { value:'powerup_clone', label:'Power-Up: Clone' },
];
...
export default function Admin(){
  ...
  // Parse URL (?game, ?mission) on first load
  useEffect(() => {
    const qs = getQS();
    if (qs.game) setActiveSlug(qs.game);
    if (qs.mission) setSelected(qs.mission);
  }, []);

  // Sync URL when game/mission changes
  useEffect(() => {
    setQS({ game: activeSlug || undefined, mission: selected || undefined });
  }, [activeSlug, selected]);
  ...
  useEffect(()=>{
    (async()=>{
      try{
        setStatus(prev => prev.startsWith('Load failed') ? '' : prev);
        const missionUrls = activeSlug
          ? [`/games/${activeSlug}/missions.json`, `/missions.json`]
          : [`/missions.json`];
        ...
        const missions = await fetchFirstJson(missionUrls, { version: '0.0.0', missions: [] });
        const conf     = await fetchJsonSafe(configUrl, defaultConfig());

        // normalize legacy 'quiz' → 'multiple_choice'
        const normalized = {
          ...missions,
          missions: (missions.missions||[]).map(x =>
            x.type==='quiz'
        ...
        setSelected(s => s); // keep current selection if any
  ...
  function defaultConfig(){
    return {
      ...
      forms:  {
        players: 1,
        collect: { firstName:true, lastName:false, email:true, phone:true } // NEW
      },
      // NEW: H2H defaults
      h2h: {
        roles: { seekerLabel: 'Seeker', hiderLabel: 'Hider' },
        reveal: { autoPingSec: 0, endChimeUrl: '' },
        smoke:  { durationSec: 45, cooldownSec: 120, stackable: true },
        clone:  { durationSec: 45, cooldownSec: 120, maxRadiusMeters: 120 },
        roleAssignment: 'player_chooses' // 'player_chooses' | 'auto_assign' | 'by_join_order'
      },
      textRules: []
    };
  }
  ...
  function defaultContentForType(t){
    switch(t){
      case 'multiple_choice': return {question:'', choices:[], correctIndex: undefined, mediaUrl:'', geofenceEnabled:false, lat:'',lng:'',radiusMeters:25,cooldownSeconds:30};
      case 'short_answer':    return {question:'', answer:'', acceptable:'', mediaUrl:'', geofenceEnabled:false, lat:'',lng:'',radiusMeters:25,cooldownSeconds:30};
      case 'statement':       return {text:'', mediaUrl:'', geofenceEnabled:false, lat:'',lng:'',radiusMeters:25,cooldownSeconds:30};
      ...
      case 'powerup_smoke':   return {lat:'',lng:'',radiusMeters:15, replenishSec:300, limitPerPlayer:2, iconUrl:''};
      case 'powerup_clone':   return {lat:'',lng:'',radiusMeters:15, replenishSec:300, limitPerPlayer:1, iconUrl:''};
      default: return {};
    }
  }
  ...
  function startNew(){
    const draft = { id: suggestId(), title:'New Mission', type:'multiple_choice', rolesAllowed: ['seeker','hider'], rewards:{points:25}, content: defaultContentForType('multiple_choice') };
  ...
  function saveToList(){
    ...
    // ensure rolesAllowed defaults to both when missing
    let rolesAllowed = editing.rolesAllowed;
    if (!Array.isArray(rolesAllowed) || rolesAllowed.length===0) rolesAllowed = ['seeker','hider'];

    // basic presence checks (ignore optional media fields)
    const fields = TYPE_FIELDS[editing.type] || [];
    for (const f of fields){
      if (f.type==='number') continue;
      const v = editing.content?.[f.key];
      if (f.key !== 'mediaUrl' && f.key !== 'iconUrl' && (v===undefined || v===null || v==='')) {
    ...
    const payload = { ...editing, rolesAllowed };
    if (i>=0) missions[i]=payload; else missions.push(payload);
    setSuite({...suite, missions, version: bumpVersion(suite.version||'0.0.0')});
    setSelected(editing.id); setEditing(null); setDirty(false);
    setStatus('✅ Added/Updated (remember Save All)');
  }
      ...
      <header style={S.header}>
        <div style={S.wrap}>
          <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
            ...
            {/* Always visible New Mission button */}
            ...
            <a href={activeSlug?`/games/${activeSlug}/config.json`:'/config.json'} target="_blank" rel="noreferrer" style={{...S.button}}>View config.json</a>
          </div>
                ...
                <div key={m.id} data-m-title={(m.title||'')+' '+m.id+' '+m.type} style={S.missionItem}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:8}}>
                    <div onClick={()=>editExisting(m)} style={{cursor:'pointer'}}>
                      ...
                      <div style={{color:'#9fb0bf',fontSize:12}}>{m.type} — id: {m.id} {Array.isArray(m.rolesAllowed)?`• ${m.rolesAllowed.join('/')}`:''}</div>
                    </div>
          ...
          <section style={S.editor}>
            {!editing ? (
              ...
              <div style={S.card}>
                ...
                {/* Roles Allowed */}
                <Field label="Roles Allowed">
                  <div style={{display:'flex',gap:12,alignItems:'center'}}>
                    <label style={{display:'flex',gap:6,alignItems:'center'}}>
                      <input
                        type="checkbox"
                        checked={editing.rolesAllowed?.includes('seeker') ?? true}
                        onChange={(e)=>{
                          const cur = new Set(editing.rolesAllowed || ['seeker','hider']);
                          if (e.target.checked) cur.add('seeker'); else cur.delete('seeker');
                          const arr = Array.from(cur); if (arr.length===0) arr.push('seeker');
                          setEditing({...editing, rolesAllowed: arr}); setDirty(true);
                        }}
                      /> Seeker
                    </label>
                    <label style={{display:'flex',gap:6,alignItems:'center'}}>
                      <input
                        type="checkbox"
                        checked={editing.rolesAllowed?.includes('hider') ?? true}
                        onChange={(e)=>{
                          const cur = new Set(editing.rolesAllowed || ['seeker','hider']);
                          if (e.target.checked) cur.add('hider'); else cur.delete('hider');
                          const arr = Array.from(cur); if (arr.length===0) arr.push('hider');
                          setEditing({...editing, rolesAllowed: arr}); setDirty(true);
                        }}
                      /> Hider
                    </label>
                ...
                {/* Geofence types map */}
                {(editing.type==='geofence_image' || editing.type==='geofence_video' || editing.type==='powerup_smoke' || editing.type==='powerup_clone') && (
                  <div style={{marginBottom:12}}>
                ...
                {/* Optional geofence: MC/SA/Statement */}
                  ...
                  <Field key={f.key} label={f.label}>
                    {f.type==='text' && (
                      <>
                        ...
                        />
                        {['mediaUrl','imageUrl','videoUrl','assetUrl','markerUrl','iconUrl'].includes(f.key) &&
      ...
      {tab==='settings' && (
        <main style={S.wrap}>
          <div style={S.card}>
              ...
              <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:8}}>
                {['firstName','lastName','email','phone'].map(k => (
                  <label key={k} style={{display:'flex',gap:8,alignItems:'center'}}>
                    <input
                      type="checkbox"
                      checked={!!config.forms.collect?.[k]}
                      onChange={e=>{
                        setConfig({
                          ...config,
                          forms: { ...config.forms, collect: { ...(config.forms.collect||{}), [k]: e.target.checked } }
                        });
                      }}
                    />
                    {k}
            ...
            {/* H2H settings */}
            {config.splash.mode === 'head2head' && (
              <>
                <hr style={S.hr}/>
                <h3 style={{marginTop:0}}>Head-to-Head Settings</h3>
                <Field label="Role Labels">
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                    <input style={S.input} placeholder="Seeker label" value={config.h2h.roles.seekerLabel}
                      onChange={e=>setConfig({...config, h2h:{...config.h2h, roles:{...config.h2h.roles, seekerLabel:e.target.value}}})}/>
                    <input style={S.input} placeholder="Hider label" value={config.h2h.roles.hiderLabel}
                      onChange={e=>setConfig({...config, h2h:{...config.h2h, roles:{...config.h2h.roles, hiderLabel:e.target.value}}})}/>
                  </div>
                </Field>
                <Field label="Role Assignment">
                  <select style={S.input} value={config.h2h.roleAssignment} onChange={e=>setConfig({...config, h2h:{...config.h2h, roleAssignment:e.target.value}})}>
                    <option value="player_chooses">Player chooses</option>
                    <option value="auto_assign">Auto assign (random)</option>
                    <option value="by_join_order">Assign by join order</option>
                  </select>
                </Field>
                <Field label="Reveal (auto ping)">
                  <div style={{display:'grid',gridTemplateColumns:'1fr 2fr',gap:8}}>
                    <input type="number" min={0} max={3600} style={S.input}
                      value={config.h2h.reveal.autoPingSec}
                      onChange={e=>setConfig({...config, h2h:{...config.h2h, reveal:{...config.h2h.reveal, autoPingSec: Number(e.target.value||0)}}})}/>
                    <input style={S.input} placeholder="End chime URL (optional)" value={config.h2h.reveal.endChimeUrl}
                      onChange={e=>setConfig({...config, h2h:{...config.h2h, reveal:{...config.h2h.reveal, endChimeUrl: e.target.value}}})}/>
                  </div>
                </Field>
                <Field label="Smoke Defaults">
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
                    <input type="number" min={1} max={600} style={S.input}
                      value={config.h2h.smoke.durationSec}
                      onChange={e=>setConfig({...config, h2h:{...config.h2h, smoke:{...config.h2h.smoke, durationSec: Number(e.target.value||0)}}})}/>
                    <input type="number" min={0} max={3600} style={S.input}
                      value={config.h2h.smoke.cooldownSec}
                      onChange={e=>setConfig({...config, h2h:{...config.h2h, smoke:{...config.h2h.smoke, cooldownSec: Number(e.target.value||0)}}})}/>
                    <label style={{display:'flex',gap:8,alignItems:'center'}}>
                      <input type="checkbox" checked={!!config.h2h.smoke.stackable}
                        onChange={e=>setConfig({...config, h2h:{...config.h2h, smoke:{...config.h2h.smoke, stackable: e.target.checked}}})}/>
                      stackable
                    </label>
                  </div>
                </Field>
                <Field label="Clone Defaults">
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
                    <input type="number" min={1} max={600} style={S.input}
                      value={config.h2h.clone.durationSec}
                      onChange={e=>setConfig({...config, h2h:{...config.h2h, clone:{...config.h2h.clone, durationSec: Number(e.target.value||0)}}})}/>
                    <input type="number" min={0} max={3600} style={S.input}
                      value={config.h2h.clone.cooldownSec}
                      onChange={e=>setConfig({...config, h2h:{...config.h2h, clone:{...config.h2h.clone, cooldownSec: Number(e.target.value||0)}}})}/>
                    <input type="number" min={10} max={1000} style={S.input}
                      value={config.h2h.clone.maxRadiusMeters}
                      onChange={e=>setConfig({...config, h2h:{...config.h2h, clone:{...config.h2h.clone, maxRadiusMeters: Number(e.target.value||0)}}})}/>
                  </div>
                </Field>
              </>
            )}
...
/** Media preview for images/videos with Dropbox/Drive normalization */
function MediaPreview({ url, kind }) {
  if (!url) return null;
  const norm = normalizeMediaUrl(url);
  const lower = norm.toLowerCase();
  const isVideo = /\.(mp4|webm|mov)(\?|#|$)/.test(lower);
  const isImage = /\.(png|jpg|jpeg|gif|webp)(\?|#|$)/.test(lower) || (!isVideo && (norm.includes('dropboxusercontent.com') || norm.includes('drive.google.com/uc')));

  return (
    <div style={{marginTop:8}}>
      <div style={{color:'#9fb0bf', fontSize:12, marginBottom:6}}>Preview ({kind})</div>
      {isVideo ? (
        <video src={norm} controls style={{width:'100%', maxHeight:260, borderRadius:10, border:'1px solid #2a323b'}} />
      ) : isImage ? (
        <img src={norm} alt="preview" style={{width:'100%', maxHeight:260, objectFit:'contain', borderRadius:10, border:'1px solid #2a323b'}} />
      ) : (
        <a href={norm} target="_blank" rel="noreferrer" style={{color:'#9fb0bf', textDecoration:'underline'}}>Open media</a>
      )}
    </div>
  );
}
...
// Try multiple URLs in order, return first valid JSON
async function fetchFirstJson(urls, fallback) {
  for (const u of urls) {
...
// Turn common sharing links into direct media URLs we can preview
function normalizeMediaUrl(u) {
  if (!u) return '';
  let url = String(u).trim();

  // Dropbox
  // - https://www.dropbox.com/s/<id>/file.png?dl=0 -> https://dl.dropboxusercontent.com/s/<id>/file.png
  // - ...?dl=1 or ...?raw=1 works too
  if (url.includes('dropbox.com')) {
    try {
      const loc = new URL(url);
      // switch host
      loc.host = 'dl.dropboxusercontent.com';
      // normalize query
      if (loc.searchParams.has('dl')) loc.searchParams.delete('dl');
      loc.searchParams.set('raw', '1');
      return loc.toString();
    } catch { /* fallthrough */ }
  }
  ...
  // You can also try "view" instead of "download" for inline display.
  const g = url.match(/https?:\/\/drive\.google\.com\/file\/d\/([^/]+)\/(view|preview)/i);
  if (g) {
    const id = g[1];
...
// Decide if URL *looks* like a video based on extension or known host hints
function isLikelyVideo(u) {
  if (!u) return false;
  const lower = u.toLowerCase();
  if (/\.(mp4|webm|mov)(\?|#|$)/.test(lower)) return true;
  // Some direct links (Drive/Dropbox) may not have file extension — let user force video by suffix
  if (lower.includes('drive.google.com') || lower.includes('dropbox')) {
    // Heuristic: if query includes "video" or filename hints
    if (/video|\.mp4|\.webm|\.mov/.test(lower)) return true;
  }
  return false;
}

// URL ↔ state helpers (deep-linking the UI)
function readUrlState() {
  if (typeof window === 'undefined') return { game: '', mission: '' };
  const sp = new URLSearchParams(window.location.search);
  return { game: sp.get('game') || '', mission: sp.get('mission') || '' };
}
function writeUrlState({ game, mission }) {
  if (typeof window === 'undefined') return;
  const sp = new URLSearchParams(window.location.search);
  if (game) sp.set('game', game); else sp.delete('game');
  if (mission) sp.set('mission', mission); else sp.delete('mission');
...
/* =========================================================
   0.5) TYPE / FORM DEFINITIONS
   - TYPE_FIELDS drives the generic renderer
   - Some types have custom UIs (MCQ, geofence maps, AR)
   ========================================================= */
...
export default function Admin(){
  ...
  const [selected, setSelected] = useState(null); // mission id selected in list
  const [editing, setEditing]   = useState(null); // draft being edited
  const [dirty, setDirty]       = useState(false);
  ...
  // Read ?game & ?mission on first mount
  useEffect(() => {
    const { game, mission } = readUrlState();
    if (game) setActiveSlug(game);
    if (mission) setPendingMissionId(mission);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Slug-aware loader for missions/config (runs on first load and when game changes)
  useEffect(()=>{
    (async()=>{
      try{
        const missionUrls = activeSlug
  ...
  // After suite loads, if URL had a mission that exists, select it once
  useEffect(() => {
    if (!pendingMissionId) return;
    const exists = suite?.missions?.some(m => m.id === pendingMissionId);
    if (exists) {
      setSelected(pendingMissionId);
      setPendingMissionId(null);
    } else {
      setPendingMissionId(null);
    }
  }, [suite, pendingMissionId]);

  // Reflect game/mission in URL
  useEffect(() => {
    writeUrlState({ game: activeSlug || '', mission: selected || '' });
  }, [activeSlug, selected]);
  ...
  function saveToList(){
    ...
    // basic presence checks for non-number fields from schema (except optional mediaUrl)
    const fields = TYPE_FIELDS[editing.type] || [];
    for (const f of fields){
      if (f.type==='number') continue;
      const v = editing.content?.[f.key];
...
/** Shows <img> or <video> preview for a URL (best-effort, with Dropbox/Drive support) */
function MediaPreview({ url, kind }) {
  const [imgOk, setImgOk] = useState(true);
  if (!url) return null;

  const normalized = normalizeMediaUrl(url);
  const treatAsVideo = isLikelyVideo(normalized);

  // For Dropbox/Drive without extensions, try image first then fallback to video
  if (!treatAsVideo && imgOk) {
    return (
      <div style={{marginTop:8}}>
        <div style={{color:'#9fb0bf', fontSize:12, marginBottom:6}}>Preview ({kind})</div>
        <img
  ...
  // Video path or image failed → try video
  return (
    <div style={{marginTop:8}}>
      <div style={{color:'#9fb0bf', fontSize:12, marginBottom:6}}>Preview ({kind})</div>
      <video src={normalized} controls style={{width:'100%', maxHeight:260, borderRadius:10, border:'1px solid #2a323b'}} />
      {!treatAsVideo && (
        <div style={{marginTop:6, fontSize:12, color:'#9fb0bf'}}>
          If this isn’t a video, open original: <a href={url} target="_blank" rel="noreferrer" style={{textDecoration:'underline', color:'#9fb0bf'}}>link</a>
        </div>
      )}
    </div>
  );
