// pages/index.jsx (patched for live identity + permissive first-run roles)
...
function AppShell() {
  ...
  const [userEmail, setUserEmail] = React.useState(() => getUserEmail());

  React.useEffect(() => {
    const handler = (e) => {
      if (e?.detail?.key === 'erix_admin_user_email_v1') {
        setUserEmail(e.detail.value || '');
      }
    };
    window.addEventListener('erix:identity_change', handler);
    return () => window.removeEventListener('erix:identity_change', handler);
  }, []);

  return (
    <div className="min-h-screen bg-white text-gray-900">
      <FirstRunModal />
      <RolesGate userEmail={userEmail} roles={roles} need="viewer" fallback={<div className="p-6 text-sm text-gray-600">Sign in to view admin (use the Welcome prompt or Settings).</div>}>
        <TopBar />
        <Tabs />
      </RolesGate>
    </div>
