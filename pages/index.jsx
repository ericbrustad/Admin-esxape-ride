...
const COOLDOWN_MS =
  (typeof process !== 'undefined' &&
    process.env &&
    Number(process.env.NEXT_PUBLIC_DEPLOY_COOLDOWN_MS)) ||
  26000;
...
export default function Admin() {
    ...
    (async () => {
      try {
        ...
        const merged = {
          ...
          devices: Array.isArray(c0.devices) ? c0.devices : Array.isArray(c0.powerups) ? c0.powerups : [],
          media: { ...(c0.media || {}) },
  ...
  function defaultContentForType(t) {
    ...
    switch (t) {
      case 'multiple_choice':
        return { question: '', choices: [], correctIndex: undefined, mediaUrl: '', ...base };
      case 'short_answer':
        return { question: '', answer: '', acceptable: '', mediaUrl: '', ...base };
      case 'statement':
        return { text: '', mediaUrl: '', ...base };
      case 'video':
        return { videoUrl: '', overlayText: '', ...base };
      case 'geofence_image':
      ...
      case 'ar_image':
        return { markerUrl: '', assetUrl: '', overlayText: '', ...base };
      case 'ar_video':
        return { markerUrl: '', assetUrl: '', overlayText: '', ...base };
  ...
  /* Save/Publish with deploy cool‑down + 409 retry + queuing */
    ...
    const id = setInterval(() => {
      if (cooldownUntil && cooldownUntil <= now()) {
        ...
        if (queuedSave) {
          setQueuedSave(false);
          saveAll();
        } else if (queuedPublish) {
          setQueuedPublish(false);
          handlePublish();
        }
      } else {
        // keep re-rendering countdown
        setStatus((s) => s);
      }
    }, 250);
    return () => clearInterval(id);
  }, [cooldownUntil, queuedSave, queuedPublish]);

  async function saveAll(attempt = 0) {
    if (!suite || !config) return;
    if (isSaving || isPublishing || inCooldown) {
      setQueuedSave(true);
      setStatus('⏳ Save queued until current deploy completes…');
      return;
    }
    setIsSaving(true);
    setStatus('Saving…');
    ...
    // Handle 409 race
    if ((!a.ok || !b.ok) && (a.status === 409 || b.status === 409) && attempt < 1) {
      setStatus('🔁 Repo moved while saving. Retrying in 5s…');
      setIsSaving(false);
      await delay(5000);
      return saveAll(attempt + 1);
    }
    ...
    setIsSaving(false);
    setStatus('✅ Saved. If /game files changed, Vercel is redeploying the Game…');
    startCooldown();
  }

  async function handlePublish(attempt = 0) {
    if (isSaving || isPublishing || inCooldown) {
      setQueuedPublish(true);
      setStatus('⏳ Publish queued until current deploy completes…');
      return;
    }
    try {
      ...
      if (res.status === 409 && attempt < 1) {
        ...
        setStatus('🔁 Repo moved while publishing. Retrying in 5s…');
        await delay(5000);
        return handlePublish(attempt + 1);
      }
  ...
  function saveToList() {
    ...
    if (i >= 0) missions[i] = obj;
    else missions.push(obj);
  ...
  function duplicateSelectedDevice() {
    if (selectedDevIdx == null) return;
    const src = devices[selectedDevIdx];
    if (!src) return;
  ...
  const canSave = !isSaving && !isPublishing && !inCooldown;
  const canPublish = !isSaving && !isPublishing && !inCooldown;
      ...
      <header style={S.header}>
        <div style={S.wrap}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button onClick={() => saveAll()} style={{ ...S.button, opacity: canSave ? 1 : 0.6 }} disabled={!canSave}>
              {isSaving ? 'Saving…' : '💾 Save All'}
            </button>
            <button
              onClick={() => handlePublish()}
              style={{ ...S.button, background: '#103217', border: '1px solid #1d5c2a', opacity: canPublish ? 1 : 0.6 }}
              disabled={!canPublish}
            >
          ...
          {/* Cooldown banner */}
          {inCooldown && <CooldownBanner until={cooldownUntil} />}
                ...
                <div key={m.id} data-m-title={(m.title || '') + ' ' + m.id + ' ' + m.type} style={S.missionItem}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 8, alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      <button title="Move up" style={{ ...S.button, padding: '6px 10px' }} onClick={() => moveMission(idx, -1)}>
                        ▲
                      </button>
                      <button title="Move down" style={{ ...S.button, padding: '6px 10px' }} onClick={() => moveMission(idx, +1)}>
                        ▼
                      </button>
                      <button title="Duplicate" style={{ ...S.button, padding: '6px 10px' }} onClick={() => duplicateMission(idx)}>
                        ⧉
                      </button>
                    </div>
                  </div>
            ...
            <div style={S.card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 12, marginBottom: 8 }}>
                <div>
                  ...
                  <div style={{ color: '#9fb0bf', fontSize: 12 }}>
                    Click to place a device (when “Add Device” is active). If a device is selected, click to move it. If none
                    selected, click moves the nearest pin.
                  </div>
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                ...
                <form onSubmit={devSearch} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 8 }}>
                  ...
                  />
                  <button type="button" style={S.button} onClick={useMyLocation}>
                    📍 My location
                  </button>
                  <button type="submit" disabled={devSearching} style={S.button}>
                    {devSearching ? 'Searching…' : 'Search'}
                  </button>
                </form>

                {/* Quick actions */}
                <div style={{ display: 'flex', gap: 8 }}>
                  <button style={S.button} onClick={addDevice}>
                    + Add Device
                  </button>
                  <button style={S.button} disabled={selectedDevIdx == null} onClick={duplicateSelectedDevice}>
                    ⧉ Duplicate
                  </button>
                  <button style={S.button} disabled={selectedDevIdx == null} onClick={deleteSelectedDevice}>
                    🗑 Delete
                  </button>
                </div>
                ...
                >
                  {devResults.map((r, i) => (
                    ...
                    >
                      ...
                      <div style={{ color: '#9fb0bf', fontSize: 12 }}>
                        lat {Number(r.lat).toFixed(6)}, lng {Number(r.lon).toFixed(6)}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {placingDev && (
                <div style={{ border: '1px solid #22303c', borderRadius: 10, padding: 10, marginBottom: 8 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8 }}>
                    <Field label="Title">
                      <input style={S.input} value={devDraft.title} onChange={(e) => setDevDraft((d) => ({ ...d, title: e.target.value }))} />
                    </Field>
                    <Field label="Type">
                      <select
                        style={S.input}
                    ...
                    <Field label="Effect (sec)">
                      <input
                        ...
                        onChange={(e) =>
                          setDevDraft((d) => ({ ...d, effectSeconds: clamp(Number(e.target.value || 0), 5, 3600) }))
                        }
                      />
                    </Field>
                  </div>
                  <div style={{ marginTop: 8, display: 'flex', gap: 8, alignItems: 'center' }}>
                    <button style={S.button} onClick={() => setPlacingDev(false)}>
                      Cancel
                    </button>
                    <button style={S.button} onClick={saveDraftDevice}>
                      Save Device
                    </button>
                    <div style={{ color: '#9fb0bf' }}>
                      {devDraft.lat == null
                        ? 'Click the map or search an address to set location'
              ...
              <MapOverview
                ...
                draftDevice={
                  placingDev ? { lat: devDraft.lat, lng: devDraft.lng, radius: devDraft.pickupRadius } : null
                }
              ...
              <div style={S.overlay}>
                <div style={{ ...S.card, width: 'min(820px, 92vw)', maxHeight: '80vh', overflowY: 'auto' }}>
                  ...
                  {editing.type === 'stored_statement' && (
                    <StoredStatementComposer editing={editing} setEditing={setEditing} setDirty={setDirty} suite={suite} />
                  )}
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <input
                      ...
                      onChange={(e) => {
                        setEditing({ ...editing, appearanceOverrideEnabled: e.target.checked });
                        setDirty(true);
                      }}
                    />
                  ...
                  {editing.appearanceOverrideEnabled && (
                    <AppearanceEditor
                      ...
                      onChange={(next) => {
                        setEditing({ ...editing, appearance: next });
                        setDirty(true);
                      }}
                    />
                  )}
      ...
      {tab === 'settings' && (
        <main style={S.wrap}>
          ...
          <div style={{ ...S.card, marginTop: 16 }}>
            ...
            <AppearanceEditor value={config.appearance || defaultAppearance()} onChange={(next) => setConfig({ ...config, appearance: next })} />
      ...
      {tab === 'devices' && (
        <main style={S.wrap}>
          <div style={S.card}>
            ...
            {(devices || []).length === 0 && (
              <div style={{ color: '#9fb0bf' }}>No devices yet. Use “Add Device” on the Missions tab or map-side manager.</div>
            )}
        ...
        <MediaTab
          ...
          uploadToRepo={async (file, folder) => {
            const url = await (async () => {
              try {
                return await uploadToRepo(file, folder);
              } catch {
                return '';
              }
            })();
      ...
      {showNewGame && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'grid', placeItems: 'center', zIndex: 1000 }}>
          <div style={{ ...S.card, width: 420 }}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
              <button style={S.button} onClick={() => setShowNewGame(false)}>
                Cancel
              </button>
              <button
                style={S.button}
...
function CooldownBanner({ until }) {
  const [leftMS, setLeftMS] = useState(Math.max(0, until - now()));
  useEffect(() => {
    const id = setInterval(() => setLeftMS(Math.max(0, until - now())), 200);
    return () => clearInterval(id);
  }, [until]);
  const pct = Math.max(0, Math.min(100, ((COOLDOWN_MS - leftMS) / COOLDOWN_MS) * 100));
  const secs = Math.ceil(leftMS / 1000);
  return (
    <div style={{ marginTop: 10, padding: 10, borderRadius: 10, border: '1px solid #274155', background: '#0e1a22', color: '#b8d1e3' }}>
      <div style={{ marginBottom: 6 }}>🚀 Deploy in progress… Save/Publish disabled for ~{secs}s</div>
      <div style={{ height: 6, background: '#0a1116', borderRadius: 6, overflow: 'hidden' }}>
        <div style={{ width: pct + '%', height: '100%', background: '#2aa4ff' }} />
      </div>
    </div>
  );
}
      ...
      <div
        style={{
          ...
          background: a.screenBgImage
            ? `linear-gradient(rgba(0,0,0,${a.screenBgOpacity}), rgba(0,0,0,${a.screenBgOpacity})), url(${toDirectMediaURL(
                a.screenBgImage
...
function StoredStatementComposer({ editing, setEditing, setDirty, suite }) {
  return (
    <div style={{ marginBottom: 12, border: '1px solid #22303c', borderRadius: 10, padding: 12 }}>
      ...
      <Field label="Template (click IDs below to insert a tag like #m03# where your cursor is)">
        <textarea
          ...
          onChange={(e) => {
            setEditing({ ...editing, content: { ...(editing.content || {}), template: e.target.value } });
            setDirty(true);
          }}
          ref={(el) => {
            if (el) editing.__tplRef = el;
          }}
        />
      </Field>
          ...
          <button
            ...
            onClick={() => {
              ...
              if (ta) {
                ...
                setEditing({
                  ...editing,
                  content: { ...(editing.content || {}), template: (editing.content?.template || '') + tag },
                });
                setDirty(true);
              }
            }}
...
function MultipleChoiceEditor({ value, correctIndex, onChange }) {
  ...
  useEffect(() => {
    setLocal(Array.isArray(value) ? value.slice(0, 5) : []);
  }, [value]);
  useEffect(() => {
    setCorrect(Number.isInteger(correctIndex) ? correctIndex : undefined);
  }, [correctIndex]);
  function sync(nextChoices, nextCorrect) {
    const trimmed = nextChoices
      .map((s) => (s || '').trim())
      .filter(Boolean)
      .slice(0, 5);
...
function MapOverview({
  ...
  function numberedIcon(number, imgUrl, color = '#60a5fa', highlight = false) {
    const img = imgUrl
      ? `<img src="${imgUrl}" style="width:24px;height:24px;border-radius:50%;object-fit:cover;border:2px solid ${
          highlight ? '#22c55e' : 'white'
        };box-shadow:0 0 0 2px #1f2937"/>`
      : `<div style="width:20px;height:20px;border-radius:50%;background:${color};border:2px solid ${
          highlight ? '#22c55e' : 'white'
        };box-shadow:0 0 0 2px #1f2937"></div>`;
  ...
  useEffect(() => {
    ...
    if (!divRef.current._leaflet_map) {
      const map = L.map(divRef.current, { center: [44.9778, -93.265], zoom: 13 });
    ...
    map._clickHandler = (e) => {
      ...
      if (selectedDevIdx != null && onMoveSelected) {
        onMoveSelected(Number(lat.toFixed(6)), Number(lng.toFixed(6)));
        return;
      }
      ...
      const candidates = [];
      (missions || []).forEach((m, idx) => {
        const p = getMissionPos(m);
        if (p) candidates.push({ kind: 'mission', idx, lat: p[0], lng: p[1] });
      });
      (devices || []).forEach((d, idx) => {
        const p = getDevicePos(d);
        if (p) candidates.push({ kind: 'device', idx, lat: p[0], lng: p[1] });
      });
  ...
  return (
    <main style={S.wrap}>
      ...
      >
        ...
        <div
          style={{
            border: '2px dashed #2a323b',
            borderRadius: 12,
            padding: 16,
            background: hover ? '#0e1116' : 'transparent',
            marginBottom: 12,
            color: '#9fb0bf',
          }}
        >
          Drag & drop files here or click <em>Choose File</em>. Files are committed to <code>public/media/…</code> and served from{' '}
          <code>/media/…</code>.
          <span style={{ float: 'right' }}>
            <FileChooser />
            <span style={{ marginLeft: 8 }}>{uploadStatus}</span>
          </span>
        </div>
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      {list.map((row, idx) => (
        <div
          key={row.key || idx}
          style={{ display: 'grid', gridTemplateColumns: '160px 1fr 1fr 140px', gap: 8, alignItems: 'center', marginBottom: 8 }}
        >
          <div>
            ...
            {row.url ? (
              <img
                alt="icon"
                src={toDirectMediaURL(row.url)}
                style={{ marginTop: 6, width: '100%', maxHeight: 72, objectFit: 'contain', border: '1px solid #2a323b', borderRadius: 8 }}
              />
            ) : (
          ...
          />
          <div style={{ display: 'flex', gap: 6 }}>
            <button style={S.button} onClick={() => { const n = [...list]; n.splice(idx, 1); setList(n); }}>
              Delete
            </button>
            <button
              style={S.button}
      ...
      <button
        style={S.button}
        onClick={() => {
          setList([...(list || []), { key: `${kind}-${list.length + 1}`, name: '', url: '' }]);
        }}
      >
        + Add Icon
      </button>
    ...
    <main style={S.wrap}>
      <div style={S.card}>
          ...
          >
            <div>
              <input
                ...
                onChange={(e) => {
                  const n = [...list];
                  n[idx] = { ...(n[idx] || {}), thumbUrl: e.target.value };
                  setList(n);
                }}
              ...
              {row.thumbUrl && (
                <img
                  alt="thumb"
                  src={toDirectMediaURL(row.thumbUrl)}
                  style={{ marginTop: 6, width: '100%', maxHeight: 80, objectFit: 'contain', border: '1px solid #2a323b', borderRadius: 8 }}
                />
              )}
            </div>
            <input
            ...
            <input
              ...
              onChange={(e) => {
                const n = [...list];
                n[idx] = { ...(n[idx] || {}), ability: e.target.value };
                setList(n);
              }}
            />
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              <button
                ...
                onClick={() => {
                  const n = [...list];
                  const copy = { ...(n[idx] || {}), key: (row.key || `rw${idx}`) + '-copy' };
                  n.splice(idx + 1, 0, copy);
                  setList(n);
                }}
              >
        ...
        <button
          style={S.button}
          onClick={() => {
            setList([...(list || []), { key: `rw${list.length + 1}`, name: '', ability: '', thumbUrl: '' }]);
          }}
        >
          + Add Reward
        </button>
...
/* TEXT tab (unchanged logic placeholder; wire to your SMS API) */
