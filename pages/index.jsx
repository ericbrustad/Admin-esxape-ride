...
export default function Admin() {
  ...
  // Rewards pickers
  const [pickForReward, setPickForReward] = useState({ open:false, which:-1, source:'game' }); // source: 'game' | 'pool'
    ...
    (async () => {
      try {
        ...
        const merged = {
          ...
          media: {
            pool: Array.isArray(c0.media?.pool) ? c0.media.pool : [],          // global refs (optional)
            gameMedia: Array.isArray(c0.media?.gameMedia) ? c0.media.gameMedia : [], // this game's media refs
            rewards: Array.isArray(c0.media?.rewards) ? c0.media.rewards : DEFAULT_REWARDS,
          },
          icons: { ...(c0.icons || {}) },
        ...
        // Load global media pool index (if exists)
  ...
  function defaultConfig() {
    return {
      ...
      textRules: [],
      devices: [], powerups: [],
      media: { pool: [], gameMedia: [], rewards: DEFAULT_REWARDS },
      icons: {},
  ...
  /* ========== DEVICES MANAGER (map-side) ========== */
  const devices = getDevices();
  function addDevice() { setPlacingDev(true); setSelectedDevIdx(null); setDevDraft({ title:'', type:'smoke', iconKey:'', pickupRadius:100, effectSeconds:120, lat:null, lng:null }); }
  function saveDraftDevice() {
    ...
    const item = {
      ...
      type: devDraft.type, iconKey: devDraft.iconKey || '',
  ...
  function deleteSelectedDevice() {
    ...
    const list=[...devices]; list.splice(selectedDevIdx,1);
    setDevices(list); setSelectedDevIdx(null);
  }
  function duplicateSelectedDevice() {
    ...
    setDevices([...(devices||[]), copy]); setSelectedDevIdx((devices?.length||0));
  }
  ...
  function setSelectedDeviceRadius(r) {
    if (selectedDevIdx == null) return;
    const list=[...devices]; list[selectedDevIdx]={ ...list[selectedDevIdx], pickupRadius: clamp(Number(r||0),1,2000) };
    setDevices(list);
  }

  // Nominatim search for devices
  async function devSearch(e) {
    e?.preventDefault();
    const q = devSearchQ.trim(); if (!q) return;
  ...
  function applySearchResult(r) {
    ...
    if (placingDev) setDevDraft(d=>({ ...d, lat, lng:lon }));
    else if (selectedDevIdx!=null) moveSelectedDevice(lat,lon);
    else { setPlacingDev(true); setDevDraft(d=>({ ...d, lat, lng:lon })); }
    setDevResults([]);
  }
  ...
  /* ========== MEDIA (Game-specific + Global Pool) ========== */
  ...
  async function uploadGameMedia(file) {
    if (!activeSlug) { setStatus('Select a game before uploading.'); return ''; }
    const url = await uploadToRepo(file, `games/${activeSlug}`);
    if (!url) return '';
    const name = file.name.replace(/[^\w.\-]+/g, '_');
    const gm = Array.isArray(config.media?.gameMedia) ? [...config.media.gameMedia] : [];
    gm.push({ name, url, thumb:url });
    setConfig({ ...config, media: { ...(config.media||{}), gameMedia: gm } });
    setStatus('✅ Added to Game Media (remember Save All)');
    return url;
  }

  // rewards image pickers
  function openRewardPicker(idx, source) { setPickForReward({ open:true, which:idx, source }); }
  function closeRewardPicker() { setPickForReward({ open:false, which:-1, source:'game' }); }
  function setRewardThumb(url) {
    const list = Array.isArray(config.media?.rewards) ? [...config.media.rewards] : [];
    const i = pickForReward.which;
    if (i >= 0 && i < list.length) {
      list[i] = { ...(list[i] || {}), thumbUrl: url };
      setConfig({ ...config, media: { ...(config.media||{}), rewards: list } });
      setStatus('✅ Reward image set (remember Save All)');
    }
    closeRewardPicker();
  }
  ...
  const gameMedia = Array.isArray(config.media?.gameMedia) ? config.media.gameMedia : [];
  const rewardsList = Array.isArray(config.media?.rewards) ? config.media.rewards : [];
    ...
    <div style={S.body}>
      ...
      {tab==='missions' && (
        <MissionsAndMap
          ...
          devices={devicesList} setDevices={setDevices}
      ...
      {/* DEVICES simple list */}
      {tab==='devices' && (
        <DevicesTab devices={devicesList} />
      )}
      ...
      {tab==='map' && (
        <main style={S.wrap}>
          <div style={S.card}>
            ...
            <MapOverview missions={(suite?.missions)||[]} devices={devicesList} icons={config.icons||{}} showRings={showRings}/>
          </div>
        </main>
      )}

      {/* MEDIA — Game media thumbnails + Global pool link */}
      {tab==='media' && (
        <MediaTab
          activeSlug={activeSlug}
          uploadStatus={uploadStatus} setUploadStatus={setUploadStatus}
          uploadGameMedia={uploadGameMedia}
          gameMedia={gameMedia}
          setGameMedia={(gm)=>setConfig({ ...config, media:{ ...(config.media||{}), gameMedia: gm } })}
          pool={pool} setPool={setPool} poolOpen={poolOpen} setPoolOpen={setPoolOpen}
        />
      )}

      {/* REWARDS — picker from link / game media / pool */}
      {tab==='rewards' && (
        <RewardsTab
          rewardsList={rewardsList}
          setRewardsList={(list)=>setConfig({ ...config, media:{ ...(config.media||{}), rewards:list } })}
          openPicker={openRewardPicker}
        />
      )}

      {/* Reward image picker modal */}
      {pickForReward.open && (
        <PickerModal
          title={pickForReward.source==='game' ? 'Pick from Game Media' : 'Pick from Media Pool'}
          items={pickForReward.source==='game' ? gameMedia : pool}
          onClose={closeRewardPicker}
          onPick={(it)=>setRewardThumb(it.url || it.thumb)}
        />
      )}

      {/* TEST */}
      {tab==='test' && (
        <TestTab gameBase={gameBase} activeSlug={activeSlug} testChannel={testChannel} setTestChannel={setTestChannel} />
      )}
      ...
      {showNewGame && (
        <NewGameModal
          ...
          newDurationMin={newDurationMin} setNewDurationMin={setNewDurationMin}
          newAlertMin={newAlertMin} setNewAlertMin={setNewAlertMin}
          ...
          onCreate={async ()=>{
            ...
            const r = await fetch('/api/games', {
              method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include',
              body: JSON.stringify({
                title:newTitle.trim(), type:newType, mode:newMode,
  ...
  return (
    <main style={S.wrap}>
      <div style={S.card}>
        ...
        {(devices||[]).length===0 && <div style={{ color:'#9fb0bf' }}>No devices yet.</div>}
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:8 }}>
                ...
                <Field label="Icon">
                  <select style={S.input} value={devDraft.iconKey} onChange={(e)=>setDevDraft(d=>({ ...d, iconKey:e.target.value }))}>
                    ...
                    {/* mission/device icons live in config.icons; optional */}
                  </select>
                </Field>
...
function MissionEditor({ editing, setEditing, dirty, setDirty, saveToList, cancelEdit, suite }) {
  if (!editing?.type || !editing?.content) return null;
  return (
    <div style={S.overlay}>
...
function MediaTab({
  ...
  pool, setPool, poolOpen, setPoolOpen,
}) {
  ...
  async function onDrop(e) {
    ...
    for (const f of files) { await uploadGameMedia(f); }
  }
    ...
    <main style={S.wrap}>
      <div style={S.card}
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <h3 style={{ marginTop:0 }}>Media (This Game)</h3>
          <div style={{ display:'flex', gap:8 }}>
            <button style={S.button} onClick={()=>setPoolOpen(true)}>📚 Media Pool</button>
            <label style={{ ...S.button, textAlign:'center' }}>
              Upload to Game
              <input type="file" multiple style={{ display:'none' }}
                onChange={async (e)=>{ const files=Array.from(e.target.files||[]); for (const f of files) await uploadGameMedia(f); e.target.value=''; }}/>
            </label>
          </div>
        </div>
        ...
        <div style={{ border:'2px dashed #2a323b', borderRadius:12, padding:16, background:hover?'#0e1116':'transparent', marginBottom:12, color:'#9fb0bf' }}>
          Drag & drop image files here to add them to this game.
          <span style={{ float:'right' }}>{uploadStatus}</span>
        </div>

        {(!gameMedia || gameMedia.length===0) ? (
          <div style={{ color:'#9fb0bf' }}>No images yet for this game.</div>
        ) : (
          <div style={gridStyles.grid}>
            {gameMedia.map((it, i)=>(
              <div key={i} style={gridStyles.tile}>
                <div style={gridStyles.thumbWrap}>
                  <img src={toDirectMediaURL(it.thumb || it.url)} alt={it.name || 'image'} style={gridStyles.thumbImg}/>
                </div>
                <div style={gridStyles.name}>{it.name || it.url.split('/').pop()}</div>
                <div style={gridStyles.actions}>
                  <a href={it.url} target="_blank" rel="noreferrer"><button style={S.button}>Open</button></a>
                  <button style={{ ...S.button, background:'#3b0f12', borderColor:'#57151a' }}
                    onClick={()=>{
                      const next = [...gameMedia]; next.splice(i,1); setGameMedia(next);
                    }}>Remove</button>
                </div>
              </div>
            ))}
        ...
        {/* Pool modal just for browsing (add-to-game handled in Rewards tab) */}
        {poolOpen && (
          <PickerModal
            title="Global Media Pool"
            items={pool}
            onClose={()=>setPoolOpen(false)}
            onPick={()=>{ /* no-op pick for Media tab; use Rewards to attach */ }}
            hidePickButton
          />
        )}
      </div>
...
function RewardsTab({ rewardsList, setRewardsList, openPicker }) {
  ...
  function add() {
    const list = [...rewardsList, { key:`rw${rewardsList.length+1}`, name:'', ability:'', thumbUrl:'' }];
    setRewardsList(list);
  }
  function remove(idx) {
    const list = [...rewardsList]; list.splice(idx,1); setRewardsList(list);
  }
  function duplicate(idx) {
    const list = [...rewardsList];
    const copy = { ...(list[idx]||{}), key:(list[idx]?.key || `rw${idx}`)+'-copy' };
  ...
  return (
    <main style={S.wrap}>
      <div style={S.card}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'160px 1fr 1fr 210px', gap:8, alignItems:'center', fontSize:13, color:'#9fb0bf', marginBottom:6 }}>
          <div>Thumbnail</div><div>Name</div><div>Description</div><div>Image Source</div>
        </div>
        {rewardsList.map((row, idx)=>(
          <div key={row.key||idx} style={{ display:'grid', gridTemplateColumns:'160px 1fr 1fr 210px', gap:8, alignItems:'center', marginBottom:8 }}>
            <div>
              <div style={gridStyles.thumbWrap}><img src={toDirectMediaURL(row.thumbUrl || '')} alt="thumb" style={gridStyles.thumbImg}/></div>
            </div>
            <input style={S.input} value={row.name||''} onChange={(e)=>update(idx,{ name:e.target.value })}/>
            <input style={S.input} value={row.ability||''} onChange={(e)=>update(idx,{ ability:e.target.value })}/>
            <div style={{ display:'grid', gridTemplateColumns:'1fr', gap:6 }}>
              <input style={S.input} placeholder="Image link (https…)" value={row.thumbUrl||''} onChange={(e)=>update(idx,{ thumbUrl:e.target.value })}/>
              <div style={{ display:'flex', gap:6 }}>
                <button style={S.button} onClick={()=>openPicker(idx,'game')}>Pick Game Media</button>
                <button style={S.button} onClick={()=>openPicker(idx,'pool')}>Pick Media Pool</button>
              </div>
              <div style={{ display:'flex', gap:6 }}>
                <button style={S.button} onClick={()=>duplicate(idx)}>Duplicate</button>
                <button style={{ ...S.button, background:'#3b0f12', borderColor:'#57151a' }} onClick={()=>remove(idx)}>Delete</button>
              </div>
            </div>
          </div>
        ))}
        <button style={S.button} onClick={add}>+ Add Reward</button>
      </div>
    </main>
  );
}

function PickerModal({ title, items, onClose, onPick, hidePickButton=false }) {
  return (
    <div style={modalStyles.backdrop} onClick={onClose}>
      <div style={modalStyles.sheet} onClick={(e)=>e.stopPropagation()}>
        ...
        {(items||[]).length===0 ? (
          <div style={{ color:'#9fb0bf' }}>No images found.</div>
        ) : (
          <div style={gridStyles.grid}>
            {items.map((it, i)=>(
              <div key={i} style={gridStyles.tile}>
                ...
                <div style={gridStyles.name}>{it.name || it.url?.split('/').pop() || '(unnamed)'}</div>
                {!hidePickButton && (
                  <div style={gridStyles.actions}>
                    <button style={S.button} onClick={()=>onPick(it)}>Pick</button>
                    <a href={it.url} target="_blank" rel="noreferrer"><button style={S.button}>Open</button></a>
                  </div>
                )}
              </div>
...
/* TEST TAB */
function TestTab({ gameBase, activeSlug, testChannel, setTestChannel }) {
  return (
    <main style={S.wrap}>
      <div style={S.card}>
...
/* New Game modal */
function NewGameModal({ newTitle, setNewTitle, newType, setNewType, newMode, setNewMode, newDurationMin, setNewDurationMin, newAlertMin, setNewAlertMin, onClose, onCreate }) {
  return (
      ...
      <div style={{ ...S.card, width:420 }}>
        ...
        <Field label="Game Type">
          <select style={S.input} value={newType} onChange={(e)=>setNewType(e.target.value)}>
            {['Mystery','Chase','Race','Thriller','Hunt'].map(t=><option key={t} value={t}>{t}</option>)}
          </select>
        </Field>
        <Field label="Mode">
        ...
        <Field label="Duration (minutes — 0 = count up)">
        ...
        <div style={{ display:'flex', gap:8, marginTop:12 }}>
          <button style={S.button} onClick={onClose}>Cancel</button>
...
const gridStyles = {
  ...
  thumbWrap:{ width:'100%', aspectRatio:'4/3', background:'#0b0c10', border:'1px solid #1f262d', borderRadius:8, overflow:'hidden', marginBottom:6, display:'grid', placeItems:'center' },
...
function MapOverview({
  missions = [], devices = [], icons = {}, showRings = true,
  ...
  function numberedIcon(number, color='#60a5fa', highlight=false){
    const dot = `<div style="width:20px;height:20px;border-radius:50%;background:${color};border:2px solid ${highlight?'#22c55e':'white'};box-shadow:0 0 0 2px #1f2937"></div>`;
    return window.L.divIcon({ className:'num-pin', html:`<div style="position:relative;display:grid;place-items:center">${dot}<div style="position:absolute;bottom:-12px;left:50%;transform:translateX(-50%);font-weight:700;font-size:12px;color:#fff;text-shadow:0 1px 2px #000">${number}</div></div>`, iconSize:[24,28], iconAnchor:[12,12] });
  }
  ...
  useEffect(()=>{
    ...
    (missions||[]).forEach((m,idx)=>{
      ...
      const mk=L.marker(pos,{icon:numberedIcon(idx+1,'#60a5fa',false)}).addTo(layer);
    ...
    (devices||[]).forEach((d,idx)=>{
      ...
      const mk=L.marker(pos,{icon:numberedIcon(`D${idx+1}`,'#f59e0b', selectedDevIdx===idx)}).addTo(layer);
...
function MediaPreview({ url, kind }) {
  ...
  const isImage = /\.(png|jpg|jpeg|gif|webp)(\?|#|$)/i.test(u) || u.includes('drive.google.com/uc?export=view');
  return (
