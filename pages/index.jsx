...
function defaultAppearance() {
  return {
    ...
    screenBgImage: '',
    textAlign: 'center',
    textVertical: 'top', // keep backpack visible
  };
}
...
export default function Admin() {
  ...
  async function saveAll() {
    ...
    else     setStatus('✅ Saved (files committed). Vercel will redeploy the Game project if files under /game changed.');
  }
  ...
  function onMoveNearest(kind, index, lat, lng) {
    // used by MapOverview single-click
    if (kind === 'mission') {
      const list = [...(suite?.missions || [])];
    ...
    return (
      <main style={{ maxWidth: 900, margin: '40px auto', color: '#9fb0bf', padding: 16 }}>
        <div style={{ padding: 16, borderRadius: 12, border: '1px solid #1f262d', background: '#12181d' }}>
          Loading… (first load pulls config & missions)
        </div>
      </main>
    );
  }
...
function MapOverview({
  ...
  function iconUrl(kind,key){ if(!key)return''; const list=icons?.[kind]||[]; const it=list.find(x=>x.key===key); return it?toDirect(it.url||''):''; }
