// pages/index.jsx
// Admin Dashboard — Missions left, Map right, overlay editor, devices, media uploads, icons, rewards.
// Replaces "Power-Ups" with "Devices". Adds numbering, reordering, duplicate, alt/option-click to move nearest pin,
// drag+drop uploads, icon libraries, and mission/device icon selection.
// Appearance controls (font, size, color, background color/image, opacity, alignment) remain as implemented previously.
...
// Convert Drive/Dropbox share links to direct URLs we can embed
...
function defaultAppearance() {
  return {
    ...
    screenBgImage: '',
    textAlign: 'center',
    textVertical: 'top', // important: top avoids covering the backpack
  };
}
const DEFAULT_ICONS = {
  missions: [], // {key, name, url}
  devices:  [], // {key, name, url}
  rewards:  [], // {key, name, url}
};
...
export default function Admin() {
  ...
  // data & config
  const [suite, setSuite] = useState(null);   // {version, missions: [...]} — missions carry order
  const [config, setConfig] = useState(null); // game-wide config incl. devices, appearance, media, icons
  const [status, setStatus] = useState('');

  // editor state
  const [selected, setSelected] = useState(null); // mission id selected
  const [editing, setEditing] = useState(null);   // mission being edited (overlay)
  const [dirty, setDirty] = useState(false);

  // devices (inline placement mode)
  ...
  // for uploads preview feedback
  const [uploadStatus, setUploadStatus] = useState('');

  // compute game origin (after config exists to avoid TDZ on build)
  const gameBase =
    ((typeof window !== 'undefined'
      ? (window.__GAME_ORIGIN__ || process.env.NEXT_PUBLIC_GAME_ORIGIN)
      : process.env.NEXT_PUBLIC_GAME_ORIGIN) || (config?.gameOrigin) || '');

  // convenience alias — support legacy config.powerups while exposing config.devices
  ...
  /* ── load missions + config for selected game ── */
  useEffect(() => {
    (async () => {
      try {
        setStatus('Loading…');
        ...
        const merged = {
          ...
          icons: { ...(c0.icons || {}), ...DEFAULT_ICONS }, // legacy safe
  ...
  function defaultConfig() {
    return {
      ...
      textRules: [],
      devices: [],
      powerups: [], // legacy path; we keep mirrored
      media: {},    // uploads, thumbnails, etc
      icons: DEFAULT_ICONS,
  ...
  /* ── Missions CRUD / reorder / duplicate ── */
  function suggestId() {
    const base = 'm';
    let i = 1;
  ...
  function startNew() {
    const draft = {
      ...
      iconKey: '', // mission pin icon (optional)
      rewards: { points: 25 },
  ...
  function saveToList() {
    ...
    const saveObj = { ...editing };
    if (!saveObj.appearanceOverrideEnabled) delete saveObj.appearance;
    if (i >= 0) missions[i] = saveObj;
    else missions.push(saveObj);
  ...
  /* ── Alt/Option click to move nearest pin (mission or device) ── */
      ...
      <div style={{ border:'1px solid #22303c', borderRadius:10, padding:12 }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap:12 }}>
          ...
          <Field label="Font size (px)">
            <input type="number" min={10} max={72} style={S.input} value={a.fontSizePx}
  ...
  return (
    <div style={S.body}>
      ...
      <header style={S.header}>
        <div style={S.wrap}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <a href={activeSlug ? `/games/${encodeURIComponent(activeSlug)}/missions.json` : '/missions.json'}
               target="_blank" rel="noreferrer" style={{ ...S.button }}>View missions.json</a>
            <a href={activeSlug ? `/api/config?slug=${encodeURIComponent(activeSlug)}` : '/config.json'}
               target="_blank" rel="noreferrer" style={{ ...S.button }}>View config.json</a>
          </div>
          <div style={{ color: '#9fb0bf', marginTop: 6, whiteSpace: 'pre-wrap' }}>{status}</div>
        </div>
      </header>

      {/* MISSIONS tab — left list, right map. New Mission editor overlays on the map side */}
      {tab === 'missions' && (
        <main style={S.wrapGrid2}>
          {/* LEFT: numbered mission list with delete on left, arrows + duplicate on right */}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 8, alignItems:'center' }}>
                    ...
                    <div onClick={() => editExisting(m)} style={{ cursor: 'pointer' }}>
                      <div style={{ fontWeight: 600 }}>
                        <span style={{ opacity:.7, marginRight:6 }}>#{idx+1}</span>
                        {m.title || m.id}
                      </div>
          ...
          {/* RIGHT: Overview Map with device placement panel; Mission editor overlay on top of map */}
            ...
            <div style={S.card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 12, marginBottom: 8 }}>
                <div>
                  ...
                  <div style={{ color:'#9fb0bf', fontSize:12 }}>
                    Click to place devices (when placing mode is active). <b>Option/Alt‑click</b> moves the nearest mission/device pin.
              ...
              {placingDev && (
                <div style={{ border:'1px solid #22303c', borderRadius:10, padding:12, marginBottom:10 }}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:12 }}>
                    <Field label="Title">
                      <input style={S.input} value={devDraft.title} onChange={(e)=>setDevDraft({ ...devDraft, title: e.target.value })}/>
                    </Field>
                    <Field label="Type">
                  ...
                  <div style={{ color:'#9fb0bf', fontSize:12, marginTop:6 }}>
                    {devDraft.lat==null ? 'Click the map to place pin.' : `lat ${Number(devDraft.lat).toFixed(6)}, lng ${Number(devDraft.lng).toFixed(6)}`}
                  </div>
                </div>
              )}
              ...
              <MapOverview
                ...
                onAltMoveNearest={onAltMoveNearest}
              />
            </div>

            {/* Mission editor overlay (shows on top of map area when editing) */}
            {editing && (
              <div style={S.overlay}>
                <div style={{ ...S.card, width:'min(780px, 92vw)', maxHeight:'80vh', overflowY:'auto' }}>
                          ...
                          <Field label="Cooldown (sec)">
                            <input type="number" min={0} max={3600} style={S.input}
                              ...
                              onChange={(e)=>{ const v = Number(e.target.value || 0);
                                setEditing({ ...editing, content: { ...editing.content, cooldownSeconds: v } }); setDirty(true); }}
                            />
                          </Field>
                        </>
                      )}
                  ...
                  {/* Generic field renderer + previews */}
        ...
        <main style={S.wrap}>
          <div style={S.card}>
            ...
            <Field label="Game Title">
              <input style={S.input} value={config.game.title}
            ...
            <Field label="Mode (affects players on splash)">
              <select style={S.input} value={config.splash.mode}
                onChange={(e)=> {
          ...
          <div style={{ ...S.card, marginTop: 16 }}>
            ...
            <div style={{ color:'#9fb0bf', marginTop:8, fontSize:12 }}>
              Tip: Keep vertical alignment on <b>Top</b> so text doesn’t cover the backpack in gameplay.
            </div>
          </div>
        </main>
      )}
      ...
      {/* DEVICES (list view; add from Missions map) */}
      {tab === 'devices' && (
        <main style={S.wrap}>
          <div style={S.card}>
            <h3 style={{ marginTop: 0 }}>Devices</h3>
            {(getDevices() || []).length === 0 && <div style={{ color: '#9fb0bf' }}>No devices yet. Use “+ Add Device” on the Missions tab to place one on the map.</div>}
      ...
      {tab === 'map' && (
        <main style={S.wrap}>
          <div style={S.card}>
            ...
            <MapOverview missions={(suite?.missions)||[]} devices={getDevices()} icons={config.icons||DEFAULT_ICONS} showRings={showRings} />
      ...
      {/* REWARDS — separate tab */}
      {tab === 'rewards' && (
        <RewardsTab config={config} setConfig={setConfig} />
      )}
...
function MediaTab({ config, setConfig, uploadStatus, setUploadStatus }) {
  /* Upload helpers (client → base64 → /api/upload to repo under public/media/) */
  async function uploadFileToRepo(file, subfolder='uploads') {
  ...
  function UrlRow({ label, value, onChange, hint }) {
    const [localFile, setLocalFile] = useState(null);
    return (
      <div style={{ display:'grid', gridTemplateColumns:'140px 1fr auto', gap:8, alignItems:'center', marginBottom:8 }}>
        <div style={{ color:'#9fb0bf' }}>{label}</div>
        <input style={S.input} value={value} onChange={(e)=>onChange(e.target.value)} placeholder={hint||'https://...'}/>
        <label style={{ ...S.button, display:'inline-block' }}>
          Choose File
          <input type="file" style={{ display:'none' }}
            onChange={async (e)=>{ const f=e.target.files?.[0]; if (!f) return;
              const url = await uploadFileToRepo(f, 'assets');
              if (url) onChange(url);
            }}/>
        </label>
      </div>
      ...
      <div style={{ marginTop:12 }}>
        ...
        {list.map((row, idx)=>(
          <div key={row.key || idx} style={{ display:'grid', gridTemplateColumns:'160px 1fr 1fr 140px', gap:8, alignItems:'center', marginBottom:8 }}>
            <div>
              <UrlRow label="" value={row.url||''} onChange={(v)=>{ const n=[...list]; n[idx]={ ...(n[idx]||{}), url:v }; setList(n); }} hint="icon image URL" />
              {row.url ? <img src={toDirectMediaURL(row.url)} alt="icon" style={{ width:'100%', maxHeight:72, objectFit:'contain', border:'1px solid #2a323b', borderRadius:8 }}/> : <div style={{ color:'#9fb0bf' }}>No image</div>}
            </div>
  ...
  return (
    <main style={S.wrap}>
      <div style={S.card}>
        ...
        {/* Generic Uploads / Drag and Drop */}
        <div style={{ border:'1px dashed #2a323b', borderRadius:12, padding:12, background:'#0e1116', marginBottom:12 }}
             onDragOver={(e)=>e.preventDefault()}
             onDrop={async (e)=>{ e.preventDefault();
               const files = Array.from(e.dataTransfer.files||[]);
               for (const f of files) await uploadFileToRepo(f, 'uploads');
             }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:12 }}>
            <div>
              <b>Uploads</b> — drag & drop files here, or use the buttons in rows below. Files are committed to <code>public/media/…</code>
              and served from <code>/media/…</code>.
            </div>
            <div style={{ color:'#9fb0bf' }}>{uploadStatus}</div>
          </div>
        </div>

        {/* Icons libraries */}
        <IconList title="Mission Icons" kind="missions" />
        <IconList title="Device Icons"  kind="devices" />
        <IconList title="Reward Icons"  kind="rewards" />
      </div>
    </main>
  );
}
  ...
  return (
    <main style={S.wrap}>
      <div style={S.card}>
        ...
        {list.map((row, idx)=>(
          <div key={row.key || idx} style={{ display:'grid', gridTemplateColumns:'160px 1fr 1fr 140px', gap:8, alignItems:'center', marginBottom:8 }}>
            <div>
              ...
              {row.thumbUrl && <img src={toDirectMediaURL(row.thumbUrl)} alt="thumb" style={{ marginTop:6, width:'100%', maxHeight:80, objectFit:'contain', border:'1px solid #2a323b', borderRadius:8 }}/>}
            </div>
            <input style={S.input} value={row.name||''} onChange={(e)=>{ const n=[...list]; n[idx]={ ...(n[idx]||{}), name:e.target.value }; setList(n); }}/>
        ...
        <button style={S.button} onClick={()=>{ const n=[...list, { key:`rw${list.length+1}`, name:'', ability:'', thumbUrl:'' }]; setList(n); }}>+ Add Reward</button>
      </div>
    </main>
  );
}
...
function MapPicker({ lat, lng, radius, onChange }) {
  ...
  useEffect(() => {
    ...
    if (!mapRef.current) {
      ...
      const p = defaultPos;
      markerRef.current.setLatLng(p); circleRef.current.setLatLng(p); circleRef.current.setRadius(Number(r||25));
    }
...
/* ──────────────────────────────────────────────────────────────────────────
   MapOverview — missions + devices pins (with numbers & icons). Alt/Option
   click to move nearest pin. Draft device shown when interactive=true.
   ────────────────────────────────────────────────────────────────────────── */
function MapOverview({
  ...
  function makeNumberedIcon(number, imgUrl, color='#60a5fa') {
    // Numbered pin with optional image background.
    const img = imgUrl ? `<img src="${imgUrl}" style="width:24px;height:24px;border-radius:50%;object-fit:cover;border:2px solid white;box-shadow:0 0 0 2px #1f2937"/>`
                       : `<div style="width:20px;height:20px;border-radius:50%;background:${color};border:2px solid white;box-shadow:0 0 0 2px #1f2937"></div>`;
    return window.L.divIcon({
      className:'num-pin',
      html:`<div style="position:relative;display:grid;place-items:center">${img}<div style="position:absolute;bottom:-12px;left:50%;transform:translateX(-50%);font-weight:700;font-size:12px;color:#fff;text-shadow:0 1px 2px #000">${number}</div></div>`,
  ...
  React.useEffect(() => {
    ...
    // Missions with numbers 1..N
    (missions||[]).forEach((m, idx) => {
      const pos = getMissionPos(m);
      if (!pos) return;
      const url = findIconUrl('missions', m.iconKey);
      const icon = makeNumberedIcon(idx+1, url, '#60a5fa');
      const mk = L.marker(pos, { icon }).addTo(layer);
      const rad = Number(m.content?.radiusMeters || 0);
      if (showRings && rad > 0) L.circle(pos, { radius: rad, color: '#60a5fa', fillOpacity: 0.08 }).addTo(layer);
      bounds.extend(pos);
    });
    ...
    (devices||[]).forEach((d, idx) => {
      ...
      if (!pos) return;
      const url = findIconUrl('devices', d.iconKey);
      const icon = makeNumberedIcon(`D${idx+1}`, url, '#f59e0b');
      const mk = L.marker(pos, { icon }).addTo(layer);
      const rad = Number(d.pickupRadius || 0);
      if (showRings && rad > 0) L.circle(pos, { radius: rad, color: '#f59e0b', fillOpacity: 0.08 }).addTo(layer);
      bounds.extend(pos);
    ...
    if (draftDevice && typeof draftDevice.lat === 'number' && typeof draftDevice.lng === 'number') {
      const pos = [draftDevice.lat, draftDevice.lng];
      const icon = makeNumberedIcon('D+', '', '#34d399');
      const mk = L.marker(pos, { icon, draggable:true }).addTo(layer);
      mk.on('dragend', () => {
        const p = mk.getLatLng();
        onDraftChange && onDraftChange(Number(p.lat.toFixed(6)), Number(p.lng.toFixed(6)));
      });
    ...
    // Option/Alt-click moves nearest pin (mission or device)
    if (!map._altClickHandlerAttached) {
      map.on('click', (e) => {
        ...
        if (best) onAltMoveNearest && onAltMoveNearest(best.kind, best.idx, e.latlng.lat, e.latlng.lng);
      });
      map._altClickHandlerAttached = true;
    }

    if (bounds.isValid()) map.fitBounds(bounds.pad(0.2));
  }, [leafletReady, missions, devices, icons, showRings, interactive, draftDevice, onAltMoveNearest, onDraftChange]);
