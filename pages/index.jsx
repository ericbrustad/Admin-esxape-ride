...
export default function Admin() {
  ...
  const currentVersionUrl = useMemo(() => {
    if (!gameBase) return '';
    const base = gameBase.replace(/\/+$/, '');
    if (!base) return '';
    if (!activeSlugForClient) return '';
    return `${base}/${encodeURIComponent(activeSlugForClient)}?channel=published`;
  }, [gameBase, activeSlugForClient]);
  ...
  const copyCurrentVersionLink = async () => {
    if (!currentVersionUrl) return;
    try {
      if (typeof navigator !== 'undefined' && navigator.clipboard?.writeText) {
        await navigator.clipboard.writeText(currentVersionUrl);
      } else if (typeof document !== 'undefined') {
        const textarea = document.createElement('textarea');
        textarea.value = currentVersionUrl;
      ...
      <header style={S.header}>
        <div style={S.wrap}>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}>
            ...
            {currentVersionUrl ? (
              <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                <a href={currentVersionUrl} target="_blank" rel="noreferrer" style={{ ...S.button }}>
                  Open current version
                </a>
                <button type="button" style={S.button} onClick={copyCurrentVersionLink}>
                  Copy link
                </button>
                {linkCopied && <span style={{ color:'#9fb0bf', fontSize:12 }}>Copied!</span>}
              </div>
            ) : (!isDefault && !gameBase ? (
              <div style={{ color:'#9fb0bf', fontSize:12 }}>Set NEXT_PUBLIC_GAME_ORIGIN or gameOrigin to share a live link.</div>
            ) : null)}
          </div>
