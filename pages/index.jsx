...
const TYPE_FIELDS = {
  ...
  'video': [{ key:'videoUrl', label:'Video URL (https)', type:'text' }, { key:'overlayText', label:'Overlay Text (optional)', type:'text' }],
  'geofence_image': [
  ...
  'ar_image': [{ key:'assetUrl', label:'AR Image Asset URL', type:'text' }, { key:'markerUrl', label:'Marker URL (optional)', type:'text' }, { key:'overlayText', label:'Overlay (optional)', type:'text' }],
  'ar_video': [{ key:'assetUrl', label:'AR Video URL', type:'text' }, { key:'markerUrl', label:'Marker URL (optional)', type:'text' }, { key:'overlayText', label:'Overlay (optional)', type:'text' }],
};
...
export default function Admin(){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        ...
        const normalized = {
          ...m,
          missions: (m.missions||[]).map(x=> x.type==='quiz' ? ({...x, type:'multiple_choice', content:{
            question: x.content?.question||'', choices: x.content?.choices||[], answer: x.content?.answer||''
          }}) : x.type==='video' ? ({...x, type:'video'}) : x )
        };
        setSuite(normalized);
  ...
  function startNew(){
    const draft = { id: suggestId(), title:'New Mission', type:'multiple_choice', rewards:{points:25}, content:{question:'',choices:[],answer:''} };
  ...
  function defaultContentForType(t){
    switch(t){
      ...
      case 'ar_image': return {assetUrl:'',markerUrl:'',overlayText:''};
      case 'ar_video': return {assetUrl:'',markerUrl:'',overlayText:''};
      default: return {};
    }
  }
  function saveToList(){
              ...
              <div style={S.card}>
                ...
                {(TYPE_FIELDS[editing.type]||[]).map(f => (
                  <Field key={f.key} label={f.label}>
                    {f.type==='text' && (<input style={S.input} value={editing.content?.[f.key]||''} onChange={e=>{ setEditing({...editing, content:{...editing.content,[f.key]:e.target.value}}); setDirty(true); }}/>)}
                    {f.type==='number' && (<input type="number" min={f.min} max={f.max} style={S.input} value={editing.content?.[f.key]??''} onChange={e=>{ const v=e.target.value===''? '': Number(e.target.value); setEditing({...editing, content:{...editing.content,[f.key]:v}}); setDirty(true); }}/>)}
                    {f.type==='multiline' && (<textarea style={{...S.input, height:120, fontFamily:'ui-monospace, Menlo'}} value={
                      f.key==='choices' ? (editing.content?.choices||[]).join('\n') : (editing.content?.[f.key]||'')
                    } onChange={e=>{
                      if (f.key==='choices'){ const lines=e.target.value.split('\n').map(s=>s.trim()).filter(Boolean); setEditing({...editing, content:{...editing.content, choices: lines}}); }
                      else { setEditing({...editing, content:{...editing.content, [f.key]: e.target.value}}); }
                      setDirty(true);
                    }}/> )}
                  </Field>
                ))}
                <Field label="Points (Reward)"><input type="number" style={S.input} value={editing.rewards?.points??0} onChange={e=>{
                  const v=e.target.value===''?0:Number(e.target.value); setEditing({...editing, rewards:{...(editing.rewards||{}), points:v}}); setDirty(true);
                }}/></Field>
      ...
      {tab==='security' && (
        <main style={S.wrap}>
          <div style={S.card}>
            ...
            <p style={{color:'#ffd166'}}>Store **Twilio** and **Vercel** credentials only as environment variables. Never in code.</p>
            <ul>
