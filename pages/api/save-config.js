export default async function handler(req, res) {
  ...
  try {
    const body = req.body || {}; const config = body.config;
    if (!config || typeof config !== 'object') return res.status(400).send('Invalid payload: expected { config }');
    ...
    const commitMessage = `chore: update config.json via admin (${new Date().toISOString()})`;
