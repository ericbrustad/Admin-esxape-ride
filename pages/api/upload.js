...
export default async function handler(req, res) {
  try {
    ...
    if (!owner || !repo || !token) return res.status(500).json({ ok:false, error:'Missing GitHub env (REPO_OWNER, REPO_NAME, GITHUB_TOKEN)' });
    ...
    if (!put.ok) return res.status(put.status).json({ ok:false, error: jr?.message || 'upload failed', response: jr });
