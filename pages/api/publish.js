...
import { readJson, bulkCommit, joinPath } from '../../src/lib/github';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!cfg && !mis) return res.status(400).json({ ok:false, error:'Nothing to publish' });
    const files = [];
    ...
    return res.status(200).json({ ok:true, wrote: files.map(f => f.path), commitUrl: commit.htmlUrl });
  } catch (err) {
