  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const devicesIn = Array.isArray(body.devices)
        ? body.devices
        : Array.isArray(body.powerups)
          ? body.powerups
          : [];
      const powerupsIn = Array.isArray(body.powerups) ? body.powerups : [];
      ...
      if (missionsIn.length) {
        ...
        if (replaceChildren) {
          ...
          const { error, count } = await supabase
            ...
            .eq('game_id', gameId)
            .eq('channel', channel)
            .not('mission_id', 'in', `(${keep.map((s) => `"${s.replace(/"/g, '\\"')}"`).join(',') || '""'})`);
      ...
      if (devicesIn.length) {
        ...
        if (replaceChildren) {
          ...
          const { error, count } = await supabase
            ...
            .eq('game_id', gameId)
            .eq('channel', channel)
            .not('id', 'in', `(${keep.map((s) => `"${s.replace(/"/g, '\\"')}"`).join(',') || '""'})`);
