...
export default async function handler(req,res){
  ...
  try{
    ...
    let base;
    if(legacy){base=channel==='draft'?joinPath('public','draft'):'public';}
    else{ if(!slug||typeof slug!=='string')return res.status(400).json({error:'Missing slug'});
  ...
  }catch(err){console.error('save error:',err);return res.status(500).json({error:String(err?.message||err)});}
}
