export default async function handler(req, res) {
  try {
    ...
    const expiresIn = Math.max(10, Math.min(3600, Number(req.query.expiresIn || 300))); // 5 min default
    ...
    if (!r.ok || !data?.signedURL) return res.status(200).json({ ok:false, error:data?.message || data || `HTTP ${r.status}` });
