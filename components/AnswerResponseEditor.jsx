// components/AnswerResponseEditor.jsx
...
// pages/index.answerresponse.patch.jsx
/*
  This is a small patch/snippet to add to your existing pages/index.jsx.
  - Create the component file above at: components/AnswerResponseEditor.jsx
  - Then in pages/index.jsx add the import, inventory fetch, and the JSX where your Mission Editor renders.

  Steps (short):
  1) Add the import near your other imports at the top:
     import AnswerResponseEditor from '../components/AnswerResponseEditor';

  2) In the Mission Editor parent component (the file that defines `editing` and `setEditing`), add these state/hooks (place them near other useState/useEffect hooks):

     // media inventory used by the editor
  ...
  3) Where you render the mission editor form (place the component where you want the "Answer Responses" UI to appear):

     <AnswerResponseEditor editing={editing} setEditing={setEditing} inventory={inventory} />

  4) When saving the mission to your backend/state, ensure you persist the onCorrect/onWrong objects from editing. Example (inside your save handler):

     function saveMission(){
       const updatedMission = { ...mission, ...editing };
       // ensure onCorrect/onWrong present
       updatedMission.onCorrect = editing.onCorrect || null;
       updatedMission.onWrong = editing.onWrong || null;
       // then save updatedMission to server or state as you normally do
     }

  5) That's it — restart dev server if needed and test the UI.
*/
