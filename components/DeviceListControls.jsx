...
export default function DeviceListControls({ devices = [], setDevices, onEdit, onDuplicate, onDelete }) {
  ...
  const itemStyle={ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px', borderRadius:8, border:'1px solid #253458', marginBottom:8, background:'#0b1115' };
  ...
  const btn={ padding:'6px 8px', borderRadius:8, border:'1px solid #253458', background:'#0b1115', color:'#f4f7ff', cursor:'pointer' };
  const small={ ...btn, padding:'4px 6px', fontSize:12 };
  return (
    <div style={{ width:'100%' }}>
      {devices.length===0 ? (<div style={{ color:'#9aa8d8', padding:8 }}>No devices yet</div>) : devices.map((d,i)=>(
        <div key={d.id||d._id||i} style={itemStyle}>
          <div style={leftStyle}>
            {d.iconUrl ? (<img src={d.iconUrl} alt='' style={iconStyle} />) : (<div style={{...iconStyle, display:'flex', alignItems:'center', justifyContent:'center', color:'#9aa8d8'}}>{d.type ? (d.type[0]||'D').toUpperCase() : 'D'}</div>)}
            <div>
              <div style={{ color:'#f4f7ff', fontWeight:600 }}>{d.title||d.name||d.label||`Device ${i+1}`}</div>
              <div style={{ color:'#9aa8d8', fontSize:12 }}>{d.type || (d.deviceType||'')}</div>
            </div>
          </div>
