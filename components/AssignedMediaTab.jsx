...
// === codex/update-admin-ui-skins-with-textures VISUALS (light/green theme) ===
...
function SmallButton({ children, onClick, tone = 'solid' }) {
  const styles =
    tone === 'danger'
      ? { background: '#fee2e2', border: '1px solid #fecaca', color: '#7a1f1f' }
      : { background: '#e6f4ea', border: '1px solid #cfe9cf', color: '#164e23' };
...
export default function AssignedMediaTab({
  ...
  // Prefer items tagged/type 'action'; else fall back to all media.
  const actionCandidates = useMemo(() => {
    const tagged = mediaPool.filter(
      (m) =>
        (m?.type && String(m.type).toLowerCase() === 'action') ||
        (Array.isArray(m?.tags) &&
          m.tags.map((t) => String(t).toLowerCase()).includes('action')),
    );
  ...
  function removeActionMedia(id) {
    const next = {
      ...safeAssigned,
      actionMedia: safeAssigned.actionMedia.filter((x) => x !== id),
    };
    onChange(next);
  }
      ...
      <Section title="Assigned Media">
        ...
        <div style={{ marginTop: 8 }}>
          <div style={{ fontWeight: 600, marginBottom: 8 }}>
            Action Media ({safeAssigned.actionMedia.length})
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {m.thumbUrl ? (
                          ...
                          <span style={{ fontSize: 12, color: '#6b7280' }}>no preview</span>
                        )}
                      </div>
                      <div>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                      <SmallButton onClick={() => window.open(m.openUrl || m.url || '#', '_blank')}>
                        Open
                      </SmallButton>
