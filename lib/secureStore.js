...
export async function getJsonFromRepo(path) {
  const r = await gh(`/repos/${OWNER}/${REPO}/contents/${encodeGitHubPath(path)}?ref=${encodeURIComponent(BRANCH)}`);
...
export async function commitJsonIfAbsent(path, obj, message) {
  ...
  const r = await gh(`/repos/${OWNER}/${REPO}/contents/${encodeGitHubPath(path)}`, {
    method: 'PUT',
