...
export function normalizeTone(tone, fallback = 'light') {
  const raw = typeof tone === 'string' ? tone.trim().toLowerCase() : '';
  if (raw === 'dark' || raw === 'night' || raw === 'd') return 'dark';
  if (raw === 'light' || raw === 'day' || raw === 'l') return 'light';
  if (raw === 'system' || raw === 'auto') {
...
export function appearanceBackgroundStyle(appearance = {}, tone = 'light') {
  ...
  const baseColor = appearance.screenBgColor || (normalizedTone === 'dark' ? '#070d18' : '#eef5ff');
  const backgroundColor = overlay > 0
    ? `rgba(${hexToRgb(baseColor)}, ${overlay})`
    : baseColor;

  const hasImage = !!appearance.screenBgImage && appearance.screenBgImageEnabled !== false;
  const backgroundImage = hasImage ? `url(${appearance.screenBgImage})` : 'none';
  const backgroundBlendMode = hasImage && overlay > 0 ? 'multiply' : '';
...
export function surfaceStylesFromAppearance(appearance = {}, tone = 'light') {
  const normalizedTone = normalizeTone(tone);
  const overlay = clamp(Number(appearance.screenBgOpacity ?? 0), 0, 1);
  const deepPanels = appearance.panelDepth !== false;

  const panelBg = normalizedTone === 'dark'
    ? `rgba(10, 16, 26, ${clamp(0.8 + overlay * 0.12, 0.72, 0.95)})`
    : `rgba(255, 255, 255, ${clamp(0.9 - overlay * 0.28, 0.56, 0.98)})`;

  const panelBorder = normalizedTone === 'dark'
    ? '1px solid rgba(132, 176, 226, 0.42)'
    : '1px solid rgba(28, 44, 64, 0.15)';

  const panelShadow = deepPanels
    ? (normalizedTone === 'dark'
      ? '0 18px 40px rgba(0, 0, 0, 0.55)'
      : '0 18px 36px rgba(12, 21, 36, 0.18)')
    : 'none';

  const pipingOpacity = deepPanels
    ? (normalizedTone === 'dark'
      ? clamp(0.28 + overlay * 0.35, 0.22, 0.62)
      : clamp(0.34 - overlay * 0.22, 0.1, 0.48))
    : 0;

  const pipingShadow = normalizedTone === 'dark'
    ? '0 0 12px rgba(0, 0, 0, 0.65)'
    : '0 0 12px rgba(13, 26, 46, 0.24)';

  return { panelBg, panelBorder, panelShadow, pipingOpacity, pipingShadow };
}
