// lib/github.js
// Admin app helpers — Repo root layout (BASE_DIR is '' in env).
// Fix: do NOT encode the full contents path (slashes must remain).
...
export function env() {
  ...
  const BASE_DIR = (process.env.GITHUB_BASE_DIR || '').replace(/^\/+|\/+$/g, ''); // '' when Admin is at repo root
...
// Read JSON relative to Admin's BASE_DIR. Returns null on 404.
export async function readJson(relPath) {
  const { OWNER, REPO, BRANCH, BASE_DIR } = env();
  const path = joinPath(BASE_DIR, relPath);
  const url = `${API}/repos/${OWNER}/${REPO}/contents/${path}?ref=${encodeURIComponent(BRANCH)}`; // DO NOT encode 'path' — keep slashes
...
// (Optional utility) list directories
...
// Single-commit writer that can write under Admin tree OR anywhere in repo via repoPath.
